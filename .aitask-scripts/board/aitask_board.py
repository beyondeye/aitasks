...
from agent_launch_utils import find_terminal, spawn_in_terminal, find_window_by_name, resolve_dry_run_command, resolve_agent_string, TmuxLaunchConfig, launch_in_tmux, load_tmux_defaults, maybe_spawn_minimonitor, _lookup_window_name, tmux_window_target
...
from followup_kinds import FOLLOWUP_KINDS, UNKNOWN_GLYPH
...
# modals only it opens — lives in board_detail_screen.py (t1794_7). Same
# re-export contract as above: `ab.TaskDetailScreen`, `ab.CycleField` … keep
# resolving (`SettingsScreen` and `KanbanApp` use `CycleField` /
# `CrossRepoRefPickerScreen` from here), but a stub of a name the EDITOR calls
# (`subprocess`, `_reload_detail_screen` …) must patch `ab.board_detail_screen`.
# The editor takes this module's helpers as required keywords;
# `make_task_detail_screen()` below binds them.
import board_detail_screen
from board_detail_screen import (
    AnchorEditScreen, AnchorField, ChildPickerItem, ChildPickerScreen,
    ChildrenField, CrossRepoDepsField, CrossRepoRefItem,
    CrossRepoRefPickerScreen, CycleField, DependencyPickerScreen, DependsField,
    DepPickerItem, FileReferenceItem, FileReferencePickerScreen,
    FileReferencesField, FoldedIntoField, FoldedTaskPickerItem,
    FoldedTaskPickerScreen, FoldedTasksField, FollowupKindField,
    FollowupKindPickerItem, FollowupKindPickerScreen, IssueField,
    LockEmailScreen, ParentField, PullRequestField, ReadOnlyField,
    RemoveDepConfirmScreen, ResetTaskConfirmScreen, TaskDetailScreen,
    UnlockConfirmScreen, VerifiesField, _reload_detail_screen,
...
# live in board_column_dialogs.py (t1794_8). Same re-export contract as above:
# `ab.ColumnManageScreen`, `ab.ColorSwatch` … keep resolving for tests and for
# the board's own push sites, but a stub of a name the DIALOGS call must patch
# `ab.board_column_dialogs`. They need no injection: the host reach is
# `self.app` (`_column_title`, `_apply_column_edit`, `_merge_source_columns`,
# `_report_merge`) and `self.manager`, both resolved at call time.
import board_column_dialogs
from board_column_dialogs import (
...
from textual.widgets import Header, Static, Label, Markdown, Input, Button, SelectionList, DataTable
...
def make_task_detail_screen(task, manager=None, read_only=False):
    """The board's `TaskDetailScreen`, bound to THIS module's helpers.

    The editor lives in board_detail_screen.py (t1794_7), which may not import
    this module (C1), so the three board helpers it calls are required keyword
    arguments. This is the one place the board binds them — the task-types
    list, the user's email and the tmux session — resolved at CALL time through
    this module's namespace, so a test stubbing `ab._get_user_email` still
    reaches the editor. `open_task_detail` and tests construct through here.
    """
    return TaskDetailScreen(
        task, manager, read_only=read_only,
        task_types_provider=lambda: _load_task_types(),
        user_email_provider=lambda: _get_user_email(),
        tmux_session_provider=lambda: _current_tmux_session(),
    )
...
from topic_semantics import (  # noqa: E402
    task_anchor_id, task_own_id, topic_key,
)
...
#
# SINCE t1794_8 THIS IS A PURE RE-EXPORT — do not "tidy" it away as unused.
# Every in-file consumer (ColumnEditScreen, ColumnManageScreen) moved to
# board_column_dialogs.py, so nothing in this module reads these names any
# more. Two things still pin them here: tests/test_board_columns_seam.py
# asserts the literal `from board_columns import` text in THIS file, and
# tests/test_board_column_manage.py reads `B.UNORDERED_ID` off the board.
from board_columns import (  # noqa: E402,F401
...
# --- Board Settings ---
# The column-customization screens this header used to cover moved to
# board_column_dialogs.py (t1794_8); SettingsScreen is board settings, not
# column management, so the section is renamed rather than left mislabelled.
...
class KanbanApp(TuiSwitcherMixin, ShortcutsMixin, TrailScreenMixin, App):
    ...
    def open_task_detail(self, task, read_only=None, source_card=None):
        ...
        self.push_screen(
            make_task_detail_screen(task, self.manager, read_only=read_only),
