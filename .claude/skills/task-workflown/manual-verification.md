...
This procedure is the interactive checklist runner that `/aitask-pickn` dispatches to when a picked task has `issue_type: manual_verification`. It is referenced from `SKILL.md` Step 3 (Check 3) and replaces Steps 6–8 for manual-verification tasks. Steps 4 (ownership) and 5 (worktree) still run before dispatch.
...
7. **Handle the answer.**
   ...
   **Abort branch** (same terminal semantics as the former "Stop here, continue later" path):
   ...
   - Inform the user: "Task t<task_id> paused at item <idx>. Re-pick with `/aitask-pickn <task_id>`."
...
- **If `DEFER > 0`:** Use `AskUserQuestion`:
  ...
  **If "Stop without archiving":** end the workflow. The task stays `Implementing` and the lock remains held so only this user can resume it. Inform the user: "Task t<task_id> left Implementing with <K> deferred items. Re-pick later with `/aitask-pickn <task_id>`."
