...
Detailed planning workflow for the task-workflown skill. Read this file when
...
1. Use `plan_verification_required = {{ profile.plan_verification_required | default(1) }}` (from profile, default 1 when absent).
2. Use `plan_verification_stale_after_hours = {{ profile.plan_verification_stale_after_hours | default(24) }}` (from profile, default 24 when absent).
3. Run:
   ```bash
...
- **Complexity Assessment:**
  ...
  - **If creating child tasks:**
    ...
    - **Child task checkpoint (ALWAYS interactive — ignores `post_plan_action` profile setting):**
      ...
      - **If "Start first child":** Restart the pick process with `/aitask-pickn <parent>_1`
      - **If "Stop here":** Display: "Child tasks and plans written to `aiplans/p<parent>/`. Pick individual children later with `/aitask-pickn <parent>_<N>`." Execute the **Satisfaction Feedback Procedure** (see `satisfaction-feedback.md`) with `skill_name` from the context variables. Then end the workflow.
...
{% if profile.risk_evaluation is defined and profile.risk_evaluation -%}
**Risk-section guard (NON-SKIPPABLE — verifies the §6.1 terminal step ran):** If `cross_repo_planned` is true, skip this guard (a cross-repo parent has no single-task `## Risk` section). Otherwise, before proceeding to the Checkpoint, confirm the externalized plan file contains `## Risk` plus both required risk headings:

```bash
grep -q '^## Risk' aiplans/<plan_file> \
  && grep -q '^### Code-health risk: \(high\|medium\|low\)' aiplans/<plan_file> \
  && grep -q '^### Goal-achievement risk: \(high\|medium\|low\)' aiplans/<plan_file> \
  && echo "RISK_OK" || echo "RISK_MISSING"
```

- `RISK_OK` → the end-of-planning Risk Evaluation ran and produced both risk dimensions; proceed to the Checkpoint.
- `RISK_MISSING` → the §6.1 End-of-planning terminal step was skipped or produced an incomplete risk section. Do **not** proceed. Re-enter plan mode (`EnterPlanMode`), run the **Risk Evaluation Procedure** and the **Risk-Mitigation design** step now, `ExitPlanMode`, and re-run **Save Plan to External File** so the complete `## Risk` section is persisted.
...
4. Commit the status revert and push:
   ...
   ./ait git push
   ```
5. Display: "Plan approved and committed. Task t\<task_num\> reverted to Ready — pick it up later with `/aitask-pickn <task_num>` in a fresh context." End the workflow (do **NOT** proceed to Step 7). The "Approve and stop here" option is always available (not profile-gated); it replaces the infeasible context-usage auto-detection by letting the user make the call based on their own HUD.
