...
Experimental staging workflow: this is a parallel copy of `task-workflow` used
by `aitask-pickn` to test hardening gates before any production workflow merge.

This skill is invoked by other skills (e.g., aitask-pickn, aitask-explore, aitask-review) after they have selected a task. The calling skill MUST establish the following context before handing off:
...
> - If child tasks were created and the child checkpoint returned "Start first child" → restart with `/aitask-pickn <parent>_1` (do NOT proceed to Step 7)
...
**Pre-implementation Risk Gate (NON-SKIPPABLE):** Before any code edits, verify the approved plan contains the full risk section produced by the Risk Evaluation Procedure:

```bash
grep -q '^## Risk' <plan_file> \
  && grep -q '^### Code-health risk: \(high\|medium\|low\)' <plan_file> \
  && grep -q '^### Goal-achievement risk: \(high\|medium\|low\)' <plan_file> \
  && echo "RISK_OK" || echo "RISK_MISSING"
```

- `RISK_OK` → proceed.
- `RISK_MISSING` → stop immediately. Do **not** edit code. Re-enter plan mode, run the **Risk Evaluation Procedure** and the **Risk-Mitigation design** step, persist the repaired plan, then re-run this gate.

**Risk fields (post-approval write):** After the Pre-implementation Risk Gate passes, parse the two decided levels from the plan headings and write them to the task's frontmatter:
...
Missing `## Risk`, missing risk headings, or missing parsed risk levels is an error, not a skip. Stop and repair the plan before continuing. This is the post-approval write gate: planning runs in read-only plan mode, so the fields are not written during Step 6.
{%- endif %}
{%- if profile.risk_evaluation is defined and profile.risk_evaluation %}
...
**Archive-time Risk Gate (NON-SKIPPABLE):** Before running `aitask_archive.sh`, verify both the approved plan and task frontmatter still contain the required risk data:

```bash
grep -q '^## Risk' <plan_file> \
  && grep -q '^### Code-health risk: \(high\|medium\|low\)' <plan_file> \
  && grep -q '^### Goal-achievement risk: \(high\|medium\|low\)' <plan_file> \
  && grep -q '^risk_code_health:[[:space:]]*\(high\|medium\|low\)' <task_file> \
  && grep -q '^risk_goal_achievement:[[:space:]]*\(high\|medium\|low\)' <task_file> \
  && echo "RISK_ARCHIVE_OK" || echo "RISK_ARCHIVE_MISSING"
```

- `RISK_ARCHIVE_OK` → proceed to archival.
- `RISK_ARCHIVE_MISSING` → stop immediately. Do **not** run `aitask_archive.sh`. Repair the plan and/or task frontmatter by rerunning the Risk Evaluation Procedure and Step 7 risk-field write, then re-run this gate.
{%- endif %}
