...
If this skill is invoked with an argument (e.g., `/aitask-reviewmode-classify security`), proceed to **Step 2** (single-file mode).

If invoked without arguments (`/aitask-reviewmode-classify`), jump to **Step 8** (batch mode).

### Step 2: Resolve File

Use fzf to fuzzy-find the argument in the reviewmodes directory:
...
Read the resolved file's full content from `aitasks/metadata/reviewmodes/<relative_path>`. Parse the YAML frontmatter to extract existing fields: `name`, `description`, `environment`, `reviewtype`, `reviewlabels`, `similar_to`.
...
```bash
cat aitasks/metadata/reviewmodes/reviewtypes.txt
```
```bash
cat aitasks/metadata/reviewmodes/reviewlabels.txt
```
```bash
cat aitasks/metadata/reviewmodes/reviewenvironments.txt
```
...
**Assign `environment`:** Determine the file's subdirectory within `aitasks/metadata/reviewmodes/`:
...
Run the comparison against all other reviewmode files:

```bash
./aiscripts/aitask_reviewmode_scan.sh --compare <relative_path>
```
...
2. If a new `reviewtype` value was used (not already in `reviewtypes.txt`):
   ```bash
   echo "<new_value>" >> aitasks/metadata/reviewmodes/reviewtypes.txt && sort -o aitasks/metadata/reviewmodes/reviewtypes.txt aitasks/metadata/reviewmodes/reviewtypes.txt
   ```
...
4. If new `environment` values were used (not already in `reviewenvironments.txt`):
   ```bash
   echo "<new_env>" >> aitasks/metadata/reviewmodes/reviewenvironments.txt && sort -o aitasks/metadata/reviewmodes/reviewenvironments.txt aitasks/metadata/reviewmodes/reviewenvironments.txt
   ```

5. **If in single-file mode** (not batch), or **batch autocommit mode**: commit all changes:
   ```bash
   git add aitasks/metadata/reviewmodes/<relative_path> aitasks/metadata/reviewmodes/reviewtypes.txt aitasks/metadata/reviewmodes/reviewlabels.txt aitasks/metadata/reviewmodes/reviewenvironments.txt
   git commit -m "ait: Classify reviewmode <filename>"
   ```

7. **If in batch non-autocommit mode**: stage changes but do not commit (Step 12 handles the commit).

**If `similar_to` was set:** Inform the user: "This file is similar to `<similar_to>`. Consider running `/aitask-reviewmode-merge <file> <similar_file>` to compare and potentially consolidate."
...
If no files are returned, inform the user: "All reviewmode files have complete metadata. Nothing to classify." and end the workflow.
...
```bash
git add aitasks/metadata/reviewmodes/ aitasks/metadata/reviewmodes/reviewtypes.txt aitasks/metadata/reviewmodes/reviewlabels.txt aitasks/metadata/reviewmodes/reviewenvironments.txt
git commit -m "ait: Classify <N> reviewmode files"
```
...
- If any similar_to pairs were found, suggest: "Consider running `/aitask-reviewmode-merge` to review merge candidates."
...
- Three vocabulary files in `aitasks/metadata/`: `reviewtypes.txt` (classification type), `reviewlabels.txt` (topic labels), `reviewenvironments.txt` (language/framework environments). New values are only added to the `aitasks/metadata/` copies — the `seed/` directory is not modified by this skill.
...
- Commit messages use the `ait:` prefix: `ait: Classify reviewmode <filename>`
