...
`aiplans/<plan_file>` is **required**. Parse the whole output and branch on the exit status per the **outcome contract** in `.claude/skills/ait-git/SKILL.md` — briefly: 0 committed, 2 nothing committed, 1 failed; and a `SKIPPED:unknown:` line naming a required path is a failure even at exit 0.
