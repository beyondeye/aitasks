...
2. **Render per-profile variant.** Run:
   `./.aitask-scripts/aitask_skill_render.sh aitask-brainstorm-discuss --profile <profile> --agent claude`
   No-op if the per-profile SKILL.md is already up to date.

3. **Dispatch via Read-and-follow.** Read the file at
   `.claude/skills/aitask-brainstorm-discuss-<profile>-/SKILL.md` and execute its
