...
**If output starts with `AUTO_SELECTED|`:** Parse the line as `AUTO_SELECTED|<filename>|<name>|<description>`. Display: "Remote mode: Using profile '\<name\>' (\<description\>)". Read the full profile: `cat aitasks/metadata/profiles/<filename>`. Store all profile fields in memory for use throughout remaining steps.
