#!/usr/bin/env bash
# test_skill_render_aitask_pr_import.sh - Regression tests for t777_12:
#   - .claude/skills/aitask-pr-import/SKILL.md.j2 (entry-point template)
#   - 4 per-agent stubs (claude/codex/gemini/opencode)
#   - 3 golden files under tests/golden/skills/aitask-pr-import/ (3 profiles, claude canonical)
# Coverage:
...
# Run: bash tests/test_skill_render_aitask_pr_import.sh
...
TEMPLATE=".claude/skills/aitask-pr-import/SKILL.md.j2"
GOLDEN_DIR="tests/golden/skills/aitask-pr-import"
...
for agent in "${AGENTS[@]}"; do
    ./.aitask-scripts/aitask_skill_render.sh aitask-pr-import --profile fast --agent "$agent" --force >/dev/null 2>&1
done

assert_contains "claude/fast: task-workflow ref rewritten under .claude/skills" \
    ".claude/skills/task-workflow-fast-/SKILL.md" "$(cat .claude/skills/aitask-pr-import-fast-/SKILL.md)"
assert_contains "codex/fast: task-workflow ref rewritten under .agents/skills" \
    ".agents/skills/task-workflow-fast-codex-/SKILL.md" "$(cat .agents/skills/aitask-pr-import-fast-codex-/SKILL.md)"
assert_contains "gemini/fast: task-workflow ref rewritten under .gemini/skills" \
    ".gemini/skills/task-workflow-fast-/SKILL.md" "$(cat .gemini/skills/aitask-pr-import-fast-/SKILL.md)"
assert_contains "opencode/fast: task-workflow ref rewritten under .opencode/skills" \
    ".opencode/skills/task-workflow-fast-/SKILL.md" "$(cat .opencode/skills/aitask-pr-import-fast-/SKILL.md)"

# === Test 5: stub-marker checks ===

echo "=== Test 5: 4 stub files contain canonical markers ==="
CLAUDE_STUB=".claude/skills/aitask-pr-import/SKILL.md"
CODEX_STUB=".agents/skills/aitask-pr-import/SKILL.md"
GEMINI_STUB=".gemini/commands/aitask-pr-import.toml"
OPENCODE_STUB=".opencode/commands/aitask-pr-import.md"

for stub in "$CLAUDE_STUB" "$CODEX_STUB" "$GEMINI_STUB" "$OPENCODE_STUB"; do
    body="$(cat "$stub")"
    assert_contains "$stub: resolve_profile uses short name 'pr-import' (t777_26)" \
        "aitask_skill_resolve_profile.sh pr-import" "$body"
    assert_not_contains "$stub: resolve_profile does NOT use full slug 'aitask-pr-import'" \
        "aitask_skill_resolve_profile.sh aitask-pr-import" "$body"
    assert_contains "$stub: skill render invocation present" \
        "aitask_skill_render.sh aitask-pr-import" "$body"
...
assert_contains "claude stub: reads from .claude/skills/aitask-pr-import-<profile>-" \
    ".claude/skills/aitask-pr-import-<profile>-/SKILL.md" "$(cat "$CLAUDE_STUB")"
assert_contains "codex stub: reads from .agents/skills/aitask-pr-import-<profile>-" \
    ".agents/skills/aitask-pr-import-<profile>-codex-/SKILL.md" "$(cat "$CODEX_STUB")"
assert_contains "gemini stub: reads from .gemini/skills/aitask-pr-import-<profile>-" \
    ".gemini/skills/aitask-pr-import-<profile>-/SKILL.md" "$(cat "$GEMINI_STUB")"
assert_contains "opencode stub: reads from .opencode/skills/aitask-pr-import-<profile>-" \
    ".opencode/skills/aitask-pr-import-<profile>-/SKILL.md" "$(cat "$OPENCODE_STUB")"
