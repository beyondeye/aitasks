...
Example: `/aitask-pickn --profile fast 42` or `/aitask-pickn 42 --profile fast`

### Step 0a: Select Execution Profile

Execute the **Execution Profile Selection Procedure** (see `.claude/skills/task-workflown/execution-profile-selection.md`) with:
- `skill_name`: `"pick"`
...
At this point, a task has been selected and confirmed. Set the following context variables, then read and follow `.claude/skills/task-workflown/SKILL.md` starting from **Step 3: Task Status Checks**:
