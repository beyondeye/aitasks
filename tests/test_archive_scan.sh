#!/usr/bin/env bash
# test_archive_scan.sh - Integration tests for archive scanner functions
# Run: bash tests/test_archive_scan.sh
...
source_scan() {
    ...
    unset _AIT_ARCHIVE_SCAN_LOADED
    unset _AIT_ARCHIVE_UTILS_LOADED
    source "$PROJECT_DIR/.aitask-scripts/lib/archive_scan.sh"
}

# --- Tests ---

echo "=== test_archive_scan.sh ==="
echo ""

# --- Test 1: scan_max_task_id -- active only ---
echo "--- Test 1: scan_max -- active only ---"
TMPDIR_1=$(setup_test_env)
echo "task" > "$TMPDIR_1/aitasks/t200_test.md"
source_scan
result=$(scan_max_task_id "$TMPDIR_1/aitasks" "$TMPDIR_1/aitasks/archived")
assert_eq "Max ID from active only" "200" "$result"
rm -rf "$TMPDIR_1"

# --- Test 2: scan_max_task_id -- archived loose ---
echo "--- Test 2: scan_max -- archived loose ---"
TMPDIR_2=$(setup_test_env)
echo "task" > "$TMPDIR_2/aitasks/archived/t300_test.md"
source_scan
result=$(scan_max_task_id "$TMPDIR_2/aitasks" "$TMPDIR_2/aitasks/archived")
assert_eq "Max ID from archived loose" "300" "$result"
rm -rf "$TMPDIR_2"

# --- Test 3: scan_max_task_id -- single numbered archive ---
...
rm -rf "$staging"
source_scan
result=$(scan_max_task_id "$TMPDIR_3/aitasks" "$TMPDIR_3/aitasks/archived")
assert_eq "Max ID from single numbered archive" "350" "$result"
rm -rf "$TMPDIR_3"

# --- Test 4: scan_max_task_id -- multiple numbered archives ---
...
rm -rf "$staging2"
source_scan
result=$(scan_max_task_id "$TMPDIR_4/aitasks" "$TMPDIR_4/aitasks/archived")
assert_eq "Max ID from multiple archives" "250" "$result"
rm -rf "$TMPDIR_4"

# --- Test 5: scan_max_task_id -- legacy fallback ---
...
# --- Test 6: scan_max_task_id -- mixed sources ---
...
# --- Test 7: scan_max_task_id -- empty ---
echo "--- Test 7: scan_max -- empty ---"
TMPDIR_7=$(setup_test_env)
source_scan
result=$(scan_max_task_id "$TMPDIR_7/aitasks" "$TMPDIR_7/aitasks/archived")
assert_eq "Max ID from empty" "0" "$result"
rm -rf "$TMPDIR_7"

# --- Test 8: search_archived_task -- found in numbered archive ---
