...
assert_exit_zero_rc  "valid pair updates the task"      "$LAST_RC"
...
assert_exit_zero_rc "clearing both succeeds" "$LAST_RC"
...
# First repopulate.
run_update --xdeps "1,2" --xdeprepo "sister"
assert_exit_zero_rc "repopulate xdeps before half-clear" "$LAST_RC"
run_update --xdeps ""
assert_exit_zero_rc "half-clear (xdeps only) succeeds, intent-only mode" "$LAST_RC"
...
run_update --xdeps "1,2" --xdeprepo "sister"
assert_exit_zero_rc "repopulate before inverse half-clear" "$LAST_RC"
run_update --xdeprepo ""
assert_exit_nonzero_rc "inverse half-clear (xdeprepo only) fails" "$LAST_RC"
