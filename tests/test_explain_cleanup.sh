...
# Create a standard test fixture with .aitask-explain directories
create_fixture() {
    local tmpdir="$1"

    # Top-level .aitask-explain runs (bare timestamps - aitask-explain style)
    mkdir -p "$tmpdir/.aitask-explain/20260225_100000"
    echo "file1.py" > "$tmpdir/.aitask-explain/20260225_100000/files.txt"

    mkdir -p "$tmpdir/.aitask-explain/20260225_120000"
    echo "file1.py" > "$tmpdir/.aitask-explain/20260225_120000/files.txt"

    mkdir -p "$tmpdir/.aitask-explain/20260226_080000"
    echo "file1.py" > "$tmpdir/.aitask-explain/20260226_080000/files.txt"

    # Codebrowser runs (keyed style)
    mkdir -p "$tmpdir/.aitask-explain/codebrowser/aiscripts__20260226_100000"
    echo "script1.sh" > "$tmpdir/.aitask-explain/codebrowser/aiscripts__20260226_100000/files.txt"

    mkdir -p "$tmpdir/.aitask-explain/codebrowser/aiscripts__20260226_110000"
    echo "script1.sh" > "$tmpdir/.aitask-explain/codebrowser/aiscripts__20260226_110000/files.txt"

    mkdir -p "$tmpdir/.aitask-explain/codebrowser/aiscripts__20260226_120000"
    echo "script1.sh" > "$tmpdir/.aitask-explain/codebrowser/aiscripts__20260226_120000/files.txt"

    # Different key - should be independent group
    mkdir -p "$tmpdir/.aitask-explain/codebrowser/tests__20260226_090000"
    echo "test1.sh" > "$tmpdir/.aitask-explain/codebrowser/tests__20260226_090000/files.txt"

    mkdir -p "$tmpdir/.aitask-explain/codebrowser/tests__20260226_100000"
    echo "test1.sh" > "$tmpdir/.aitask-explain/codebrowser/tests__20260226_100000/files.txt"

    # Single entry key - should not be removed
    mkdir -p "$tmpdir/.aitask-explain/codebrowser/imgs__20260226_120000"
    echo "logo.png" > "$tmpdir/.aitask-explain/codebrowser/imgs__20260226_120000/files.txt"
}
...
test_dry_run_no_deletion() {
    ...
    assert_dir_exists "dry run preserves old aiscripts dir" \
        "$tmpdir/.aitask-explain/codebrowser/aiscripts__20260226_100000"
    assert_dir_exists "dry run preserves old tests dir" \
        "$tmpdir/.aitask-explain/codebrowser/tests__20260226_090000"
...
test_cleanup_codebrowser_keeps_newest() {
    ...
    local output
    output=$(./.aitask-scripts/aitask_explain_cleanup.sh --target .aitask-explain/codebrowser 2>&1)

    # Newest aiscripts dir kept
    assert_dir_exists "newest aiscripts dir kept" \
        "$tmpdir/.aitask-explain/codebrowser/aiscripts__20260226_120000"
    ...
    assert_dir_not_exists "older aiscripts dir 2 removed" \
        "$tmpdir/.aitask-explain/codebrowser/aiscripts__20260226_110000"

    # Newest tests dir kept
    assert_dir_exists "newest tests dir kept" \
        "$tmpdir/.aitask-explain/codebrowser/tests__20260226_100000"
    ...
    assert_dir_exists "single entry key untouched" \
        "$tmpdir/.aitask-explain/codebrowser/imgs__20260226_120000"
...
test_cleanup_all_mode() {
    ...
    assert_dir_exists "newest bare timestamp kept" \
        "$tmpdir/.aitask-explain/20260226_080000"
    assert_dir_not_exists "older bare timestamp 1 removed" \
        "$tmpdir/.aitask-explain/20260225_100000"
    assert_dir_not_exists "older bare timestamp 2 removed" \
        "$tmpdir/.aitask-explain/20260225_120000"
...
test_all_mode_skips_codebrowser_subdir() {
    ...
    assert_dir_exists "codebrowser subdir not deleted" \
        "$tmpdir/.aitask-explain/codebrowser"
...
test_skips_dirs_without_marker_files() {
    ...
    echo "has marker" > .aitask-explain/codebrowser/aiscripts__20260226_100000/files.txt

    mkdir -p .aitask-explain/codebrowser/aiscripts__20260226_090000
    ...
    assert_dir_exists "dir without marker skipped" \
        "$tmpdir/.aitask-explain/codebrowser/aiscripts__20260226_090000"
...
test_raw_data_txt_accepted() {
    ...
    mkdir -p .aitask-explain/codebrowser/src__20260226_090000
    echo "data" > .aitask-explain/codebrowser/src__20260226_090000/raw_data.txt

    local output
    output=$(./.aitask-scripts/aitask_explain_cleanup.sh --target .aitask-explain/codebrowser 2>&1)

    assert_dir_exists "newest with raw_data.txt kept" \
        "$tmpdir/.aitask-explain/codebrowser/src__20260226_100000"
    assert_dir_not_exists "older with raw_data.txt removed" \
        "$tmpdir/.aitask-explain/codebrowser/src__20260226_090000"
...
test_unrecognized_dir_skipped() {
    ...
    mkdir -p .aitask-explain/codebrowser/aiscripts__20260226_100000
    echo "stuff" > .aitask-explain/codebrowser/aiscripts__20260226_100000/files.txt

    local output
    output=$(./.aitask-scripts/aitask_explain_cleanup.sh --target .aitask-explain/codebrowser 2>&1)

    assert_dir_exists "unrecognized dir not deleted" \
        "$tmpdir/.aitask-explain/codebrowser/some_random_dir"
...
test_nested_key_with_double_underscore() {
    ...
    echo "files" > .aitask-explain/codebrowser/aiscripts__board__20260226_100000/files.txt

    mkdir -p .aitask-explain/codebrowser/aiscripts__board__20260226_090000
    echo "files" > .aitask-explain/codebrowser/aiscripts__board__20260226_090000/files.txt

    local output
    output=$(./.aitask-scripts/aitask_explain_cleanup.sh --target .aitask-explain/codebrowser 2>&1)

    assert_dir_exists "nested key newest kept" \
        "$tmpdir/.aitask-explain/codebrowser/aiscripts__board__20260226_100000"
    assert_dir_not_exists "nested key older removed" \
        "$tmpdir/.aitask-explain/codebrowser/aiscripts__board__20260226_090000"
...
test_single_entry_per_key_no_deletion() {
    ...
    mkdir -p .aitask-explain/codebrowser/tests__20260226_090000
    echo "files" > .aitask-explain/codebrowser/tests__20260226_090000/files.txt
