...
assert_contains "codex/remote: task-workflow ref rewritten under .agents/skills" \
    ".agents/skills/task-workflow-remote-/agent-attribution.md" "$(cat .agents/skills/aitask-pickrem-remote-/SKILL.md)"
...
assert_contains "codex stub: reads from .agents/skills/aitask-pickrem-<profile>-" \
    ".agents/skills/aitask-pickrem-<profile>-/SKILL.md" "$(cat "$CODEX_STUB")"
...
for agent in "${AGENTS[@]}"; do
    ...
    committed_path="$root/aitask-pickrem-remote-/SKILL.md"
...
for agent in "${AGENTS[@]}"; do
    ...
    path="$root/aitask-pickrem-remote-/SKILL.md"
    TOTAL=$((TOTAL + 1))
    if [[ -f "$path" ]]; then
        PASS=$((PASS + 1))
    else
