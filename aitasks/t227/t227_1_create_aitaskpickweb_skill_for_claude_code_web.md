...
6. Create implementation plan at `.task-data-updated/plan_t<task_id>.md`
7. Implement the code
8. Write completion marker `.task-data-updated/completed_t<task_id>.json` with task metadata (task_id, task_file, plan_file, is_child, parent_id, issue_type, completed_at, branch). This marker is how `aitask-web-merge` detects completed branches.
9. Commit all changes (code + `.task-data-updated/` files) to current branch
...
- Verify it DOES contain: `aitask_init_data.sh`, `aitask_lock.sh --check`, `.task-data-updated/` plan and marker creation
