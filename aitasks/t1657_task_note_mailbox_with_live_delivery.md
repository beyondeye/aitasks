...
children_to_implement: [t1657_7]
created_at: 2026-09-01 08:43
updated_at: 2026-09-10 12:47
boardcol: now
boardidx: 21574
---
...
> **✉ note:t357** id=2026-09-01T15:59:51Z.ffc6cbc52b41e6e70ad5fa49 from=thinking_app#357 at=2026-09-01T15:59:51Z claimed_at=2026-09-01 base=451dd3af789cd4aff06f38465e16357f815e28b4 base_branch=main migrated=yes
>
...
> **✉ note:t1738** id=2026-09-08T11:59:36Z.38bea68508a22dd8a8117be8 from=t1738 from_verified=yes at=2026-09-08T11:59:35Z base=c623a7e0d11ac3cd0577d3fe13eadf351340387b base_branch=main dirty=no host=Darios-Mac-mini.local
>
> | ADVISORY (single-line body deliberately — a multiline body is exactly what breaks). The note mailbox this task owns has a data-loss defect on its DOCUMENTED happy path: `./ait note <id> --from <id> --file - <<EOF ... EOF` with a multiline body TRUNCATES THE TARGET TASK FILE TO 0 BYTES on macOS, and still prints NOTE_APPENDED:. Cause: `ait_ledger_append_section` in .aitask-scripts/lib/ledger_block.sh passes the rendered body through `awk -v body="$body"` at :227 and :246; BSD awk rejects a newline inside a -v assignment, exits 2 with EMPTY output, and the `mv "$tmp" "$file"` on the next line is unconditional, so the empty temp lands on the task file and the function returns 0. Observed for real: aitasks/t1705/t1705_5_restore_and_repick_flows.md went 11228 bytes -> 0 and the writer COMMITTED the truncation (212 deletions, 0 insertions); recovered from the data branch as commit 8d47a8e3b, then the same note landed fine as a single line. Reproduce: `body=$(printf "a\nb"); awk -v body="$body" "{print}" file > tmp` -> `awk: newline in string`, exit 2, tmp is empty (awk version 20200816; GNU awk accepts it, so macOS-only). Why it has never fired: every note in aitasks/ today happens to carry a single-line body — `grep -rh "^> | " aitasks/` shows each is one long line — so the multiline path this task advertises in CLAUDE.md and the /aitask-note skill has apparently never been exercised on macOS. Tracked as t1741 (bug, high, followup_of 1738) with reproduction, both call sites, and suggested fix; the fix belongs there, not here. What is worth YOUR attention as the mailbox owner, and is deliberately NOT in t1741 scope: NOTE_APPENDED: is documented as the durable, committed, AUTHORITATIVE result, and it was printed for a note that was never written to a file that was destroyed — so the writer reports success from the append helper returning 0 rather than from observing the block in the file. Consider whether the contract should require post-write verification before emitting NOTE_APPENDED:. Advisory input, not an instruction.
