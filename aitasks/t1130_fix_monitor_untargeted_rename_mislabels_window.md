...
## Root cause A — untargeted `rename-window` fallback in the monitor (this task's origin)

**Observed:** In a live `aitasks` session, an explore agent (`claude … /aitask-explore`) with its companion minimonitor pane occupied a window named `monitor` (with `automatic-rename off`). A second, legitimate `monitor` window also existed. The explore window is spawned via `tui_switcher._spawn_in_session` as `new-window -n agent-explore-N`, so it is born correctly named — it was renamed afterward.
...
This is a **t941 follow-up**: t941 (commit `bc50549d3`) pinned the rename to `$TMUX_PANE` but deliberately retained the untargeted fallback, and `tests/test_monitor_rename_window_target.sh:51-54` currently **asserts** that `None`/`''` produce the untargeted form — i.e. the test enshrines the failure mode.
...
- Update `tests/test_monitor_rename_window_target.sh` so the `None`/`''` cases assert **no rename is issued** (e.g. `_rename_window_argv` returns an empty argv / the caller skips the subprocess), converting the regression guard from blessing the bug to guarding the fix. Keep the truthy-pane assertion (`-t %7`).
- Consider adding a single-instance guard to `.aitask-scripts/aitask_monitor.sh` mirroring `.aitask-scripts/aitask_minimonitor.sh:33-42`, so a second `monitor_app` cannot start (and re-run the rename) inside a window that already hosts a monitor/minimonitor. (Secondary hardening; scope per risk gate.)
...
- The AgentCommandScreen window-reuse fix (Root cause B, merged below) is implemented with its own regression tests.

## Notes
- The uncommitted monitor diffs at investigation time (t1111_4 two-phase capture) are unrelated to this bug.


## Merged from t_fix_agent_launch_tui_window_reuse

> Root cause B — the AgentCommandScreen window-reuse path. Merged in during exploration; the original task was unnumbered and has been deleted (its full content is preserved below).
