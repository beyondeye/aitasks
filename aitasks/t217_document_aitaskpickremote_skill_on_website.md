---
priority: medium
effort: medium
depends: []
issue_type: documentation
status: Ready
labels: [website, claudeskills, documentation]
created_at: 2026-02-23 09:04
updated_at: 2026-02-23 09:04
---

Add documentation for the new aitask-pick-remote skill to the website. Include: 1) Skill reference page describing the workflow, arguments, and profile-driven configuration. 2) Document all remote-specific profile fields (done_task_action, orphan_parent_action, complexity_action, review_action, issue_action, abort_plan_action, abort_revert_status) with their values and defaults. 3) Document profile customization options and how to create custom remote profiles. 4) Explain that EnterPlanMode/ExitPlanMode are still interactive (planning step is supported). Reference: .claude/skills/aitask-pick-remote/SKILL.md for the complete skill specification.
