...
- [defer] Negative control: rerun the teardown with `AIT_NO_SYSTEMD_RUN=1` (server lands in app.slice) and confirm the SAME teardown DOES kill it — DEFER 2026-06-10 12:04 auto: placement half VERIFIED autonomously (AIT_NO_SYSTEMD_RUN=1 -> fallback server cgroup = /app.slice/...scope, NOT session.slice). Kill half MODELED: an app.slice-unit server was KILLED by the SAME stop that the session.slice server survived. Literal 'same teardown' = compositor restart (item 2) deferred as destructive; close together with item 2.
