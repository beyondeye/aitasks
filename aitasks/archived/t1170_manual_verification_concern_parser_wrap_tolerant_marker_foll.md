...
- [fail] Spawn a Codex shadow via minimonitor `e` on a plan review at a narrow pane width (~55 cols), with a concern whose region is a long full path — FAIL 2026-07-20 18:08 follow-up t1187
- [x] Confirm the picker renders the rejoined region label readably, and that forwarding the selected concern to the followed agent produces the correct `- [priority | region] body` payload — PASS 2026-07-20 18:11 user: confirmed the live picker label is readable and forwarding emits the canonical priority-region-body payload
