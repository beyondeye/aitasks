# Output from agent: explorer_001b

--- NODE_YAML_START ---
node_id: n002_explorer_001b
parents: [n000_init]
description: Steerability-preserving shadow — decision-withholding by default plus a concern-spillover ledger that defers secondary concerns into separate tasks instead of bloating the current plan.
proposal_file: br_proposals/n002_explorer_001b.md
created_at: "2026-06-17 07:52"
created_by_group: explorer

# --- Inherited from baseline (n000_init) ---
# priority: high  (baseline description field, carried as requirement)
# effort: medium
...
reference_files:
  ...
  - .claude/skills/aitask-shadow/plan-triage.md          # NEW sub-procedure
  - .aitask-scripts/aitask_shadow_capture.sh
  - .aitask-scripts/aitask_shadow_context.sh
  - .aitask-scripts/aitask_shadow_spillover.sh           # NEW helper
  - .aitask-scripts/aitask_create.sh                     # defer-to-task bridge target
  - aidocs/framework/shadow_agent.md
  - aidocs/framework/skill_authoring_conventions.md

# === Requirements (NEW dimensions) ===
requirements_fixed:
  - Advisory-only contract is inviolable — read-only wrt the followed agent; never send keystrokes/answers into its pane.
  - Shadow stays spawn-fast, static, user-invocable; no workflow-phase classifier ever becomes a gate or prerequisite.
  - The human developer remains the author of design intent and the final decision-maker.
  - Deferred concerns are never auto-applied — drafted `ait create` stubs are shown for the developer to run.
requirements_mutable:
  - Decision-withholding is the default but can be relaxed per-request ("you pick / recommend one").
  - Spillover ledger storage format and location.
  - Whether the scope-drift meter runs proactively or only on request.

# === Assumptions (NEW dimensions) ===
assumption_followed_agent_pane: A tmux pane id for the followed agent is resolvable (passed by the launcher or supplied by the user); capture works as today.
assumption_context_fetch_has_ac: The task file fetched via aitask_shadow_context.sh carries the original intent / acceptance criteria the plan can be measured against.
assumption_create_batch_available: ait create --batch can mint a child/follow-up task with depends: on the current task id without interactive prompts.
assumption_developer_owns_decisions: The developer wants to remain the decision author — the steerability premise; if they truly want full delegation that is an explicit opt-out, not the default.
assumption_no_auto_apply: Generating an ait create stub does not run it; the developer runs the command, preserving the advisory boundary even for task creation.

# === Components (NEW dimensions) ===
component_skill_flow: SKILL.md — routes the user's ask, defaults to intent-first decision-withholding, and exposes the triage/spillover capability.
component_capture: aitask_shadow_capture.sh — inherited unchanged; reads the followed agent's current screen.
component_context_fetch: aitask_shadow_context.sh — inherited; the AC/intent extraction it already returns becomes load-bearing for the scope-drift meter.
component_spillover_ledger: aitask_shadow_spillover.sh + per-task ledger file — durable home for deferred secondary concerns.
component_defer_to_task_bridge: ledger-to-`ait create --batch` stub generator — converts deferred concerns into trackable follow-up tasks.
component_decision_withholding_guardrail: tightened guardrail section — the shadow lays out the decision space and elicits the developer's intent before (optionally) recommending.
component_scope_drift_meter: lightweight comparison of the current plan against the original task AC — surfaces concerns that have crept beyond the task's stated intent.
component_triage_subprocedure: plan-triage.md — classifies each surfaced concern IN_SCOPE vs DEFER and feeds DEFER items to the ledger.

# === Tradeoffs (NEW dimensions) ===
tradeoff_advisory_purity_preserved: The defer-to-task bridge only drafts commands; the developer runs them, so task creation never breaches the advisory-only contract — but the developer must still take one explicit action to materialize each deferred task.
tradeoff_decision_withholding_friction: Default decision-withholding adds a round-trip (shadow asks for intent before recommending); mitigated by a per-request "you pick" opt-out so urgent decisions are not slowed.
tradeoff_ledger_overhead: A per-task ledger file is one more artifact to manage; mitigated by storing it under .aitask-shadow/spillover/<task_id>.md (gitignored scratch) and clearing it when the deferred tasks are created.
tradeoff_scope_meter_false_drift: AC-vs-plan comparison can mis-flag legitimately in-scope work as drift when the task AC is terse; mitigated by presenting drift as a question ("is this part of the task?") the developer answers, never an automatic defer.
tradeoff_create_stub_accuracy: Auto-drafted ait create stubs may capture a concern imperfectly; mitigated by showing the full draft for edit-before-run rather than executing it, and by carrying the concern's verbatim text in the task body.
--- NODE_YAML_END ---
--- PROPOSAL_START ---
<!-- section: overview -->
## Overview
...
--- PROPOSAL_END ---
--- NEW_DIMENSIONS ---
requirements_fixed, requirements_mutable, assumption_followed_agent_pane, assumption_context_fetch_has_ac, assumption_create_batch_available, assumption_developer_owns_decisions, assumption_no_auto_apply, component_skill_flow, component_capture, component_context_fetch, component_spillover_ledger, component_defer_to_task_bridge, component_decision_withholding_guardrail, component_scope_drift_meter, component_triage_subprocedure, tradeoff_advisory_purity_preserved, tradeoff_decision_withholding_friction, tradeoff_ledger_overhead, tradeoff_scope_meter_false_drift, tradeoff_create_stub_accuracy
