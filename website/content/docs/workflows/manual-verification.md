...
- **Item 2:** "Ctrl+N spawns a new task from the brainstorm view" → **Fail**. The helper emits `ORIGIN_AMBIGUOUS:t571_4,t571_5,t571_6`; pick `571_5` as the attribution. `FOLLOWUP_CREATED:612:aitasks/t612_fix_failed_verification_t571_7_item2.md`. The item line is now `[fail] ... follow-up t612`.
- **Item 3:** "Agent spawn lands in a fresh tmux window" → **Defer**. Waiting on the tmux wrapper landing in a sibling task.
