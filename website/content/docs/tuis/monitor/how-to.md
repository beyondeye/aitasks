...
Monitor reads the bound shadow pane, parses the block, and opens a checklist modal. Each concern is tagged with a priority (`high` / `medium` / `low`) and the plan region it refers to. A concern that priced itself adds a compact **trade profile** between the region and the body — `▲robus ▼simpl E:lo`, what it would improve, what that would cost, and the effort — and its badge shows a priority derived from that vector rather than the one the shadow typed, with a `≠` beside it when the two disagree. A concern that priced nothing renders exactly as it always has: stated priority, no `≠`, no profile. See [Read the trade profile]({{< relref "/docs/workflows/shadow-agent" >}}#read-the-trade-profile) for the glyph key. Below the list, a detail panel spells out whichever concern is focused — one line per impact entry, with full dimension names — and follows the selection as you move with ↑/↓; see [See the whole impact vector]({{< relref "/docs/workflows/shadow-agent" >}}#see-the-whole-impact-vector). Tick the ones you want, confirm, and monitor copies them with a short preamble to your clipboard. Nothing is written to the clipboard until you confirm, and monitor never types into the agent itself: you stay the driver.

**The decision reminder.** When the block carries at least one priced concern and the picker has room — roughly 80 columns and 24 rows, which a full-terminal monitor normally has — a one-line rule sits above the list: forward on an obligation or a pure win, spin off what is net-positive or costly, reject where the worsen side is at least as large as the improve side. Below that size the key hints take precedence and the reminder is dropped; the per-row profile is unaffected. A monitor in a narrow terminal also falls back to the multi-line row form, and it does so by measuring its own width rather than from any fixed assumption about which TUI it is.
...
**Rejecting a concern.** Each row carries one of four mutually exclusive dispositions: unmarked (`□`), marked to forward with **Space** (`✓`), marked **rejected** with **r** (`✗`, and the row dims), or marked to **spin off** with **t** (`»`). Rejecting says *stop raising this for this task*, so the shadow drops it from later review rounds — see [Reject a concern so it does not come back]({{< relref "/docs/workflows/shadow-agent" >}}#reject-a-concern-so-it-does-not-come-back) for what the shadow does with it.

**Spinning a concern off.** Press **t** to mark a concern `»` — *keep this, but as its own task*. On confirmation each marked concern becomes a **draft** in `aitasks/new/`, carrying the concern's priority, a `shadow-concern` label, and a link back to the task under review. Drafts claim no task number and commit nothing; monitor reports the paths it wrote plus a selector for the batch, and you finalize the ones you want with `ait create`. A spun-off concern is also recorded as handled, so the shadow stops raising it in later rounds — it is tracked elsewhere now. Unlike a rejection the row is not dimmed: the concern is being kept. This is the answer to a review whose secondary findings would otherwise bloat the plan.
...
> **These keys belong to the picker, not to monitor.** Inside the concern modal, **r** rejects, **t** spins a concern off, **e** edits the outgoing payload, and **R** opens the rejected list. Monitor's own **r** (refresh), **t** (scroll the preview to its tail), **e** (launch shadow) and **R** (restart task) are not reachable while the modal is open, so there is no conflict — but the same key does different things depending on whether the picker has focus.
...
> **Badge and auto-offer:** every agent whose shadow has an un-picked block is marked with `◆!` on its card, so nothing is missed across many agents at once. A toast — `Shadow raised 2 concern(s) — press 'c' to pick` — fires only for the **currently selected** agent, so there is at most one popup no matter how many agents are running. The count is of concerns needing attention; any informational ones are noted separately in the same toast, and `(⚠ STALE — agent moved on)` is appended when the agent has produced output since the shadow last analyzed it. The review round is named too, as a `(round N)` suffix, when the block carries one. A block whose markers cannot be parsed still raises the badge but deliberately does not toast — pressing **c** then tells you exactly what went wrong.
>
> **A new round re-offers concerns you have already seen.** Each review round re-derives the shadow's findings from scratch, and the offer is keyed on the round as well as the concerns. A repeat round raising an identical list means the shadow re-reviewed and stands by it — news, not noise.
>
> **A round that cannot be read is never reported as "no concerns".** A review finding nothing still records the round, and **c** reports it as a clean round by number. But a block that carries a round and yields no readable concerns — still mid-stream, or with content the parser had to drop — gets a warning and the **raw block** instead, because an all-clear there would hide output the shadow did produce.
