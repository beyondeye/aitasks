...
> **These keys belong to the picker, not to monitor.** Inside the concern modal, **r** rejects, **t** spins a concern off, and **R** opens the rejected list. Monitor's own **r** (refresh), **t** (scroll the preview to its tail) and **R** (restart task) are not reachable while the modal is open, so there is no conflict — but the same key does different things depending on whether the picker has focus.
