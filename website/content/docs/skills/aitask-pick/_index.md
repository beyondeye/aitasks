...
5. **Assignment** — Tracks who is working on the task via email, sets status to "Implementing", commits and pushes the status change. If the task was already `Implementing` and assigned to you (e.g., a prior agent crashed mid-implementation), routes through the [Crash Recovery](../../workflows/crash-recovery/) flow before continuing
6. **Branch resolution** — Resolves the base branch and the merge target, and decides whether the task gets its own git branch and worktree (`aiwork/<task_name>/`) or works directly on the current branch. Nothing is created at this point
...
8. **Implementation** — First runs the [parallel-admission preflight](parallel-admission/), which reports whether any other in-flight task declares the files this plan touches; it is advisory and never stops the workflow on its own. Then asks the project's optional [resource-admission hook](resource-admission/) whether this host can afford to start; a refusal parks the task with its approved plan and ends the session, before anything is created. Then, when a worktree was chosen in phase 6, creates the branch and `aiwork/<task_name>/` worktree, so the fork point is the base branch as it stands once the plan is approved and the remote drift check has passed. Then follows the approved plan, updating the plan file with progress and any deviations
