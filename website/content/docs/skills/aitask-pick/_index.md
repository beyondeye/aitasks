...
8. **Implementation** — When a worktree was chosen in phase 6, creates the branch and `aiwork/<task_name>/` worktree first, so the fork point is the base branch as it stands once the plan is approved and the remote drift check has passed. Then follows the approved plan, updating the plan file with progress and any deviations
