...
| `parallel_admission` | string | `"confirm"`, `"warn"` (default), or `"off"` — how loudly the [parallel-admission preflight](../parallel-admission/) surfaces a non-CLEAR verdict. **No value ever stops the workflow.** Quote `"off"`: YAML reads a bare `off` as false. Headless profiles must set it. All three shipped profiles opt out; [why](../parallel-admission/#why-all-three-shipped-profiles-ship-off) |
| `parallel_assessment` | string | `"off"` (default, and what an absent key means), `"show"`, or `"ask"` — opt into the [pre-claim parallel-safety assessment](../parallel-admission/#pre-claim-assessment-opt-in), which runs before the task is claimed. Disabled costs nothing: no checker call, no reading, no prompt. Headless profiles never prompt in either mode |
