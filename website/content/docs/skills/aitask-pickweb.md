...
**Fields from `/aitask-pickrem` that are ignored** (not applicable in web mode): `default_email`, `force_unlock_stale`, `done_task_action`, `orphan_parent_action`, `review_action`, `issue_action`, `abort_plan_action`, `abort_revert_status`, `create_worktree`, `base_branch`, `output_branch`, `enableFeedbackQuestions`.
