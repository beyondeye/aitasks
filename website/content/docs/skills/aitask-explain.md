...
- **Run reuse** — Generated reference data is stored in `.aitask-explain/<dir_key>__<timestamp>/` and can be reused across sessions. When you return to a file, select "Use existing analysis" to skip regeneration. Data can also be refreshed if the file has changed
...
Run data is stored under `.aitask-explain/` with one directory per run, using the naming convention `<dir_key>__<timestamp>` (e.g., `aiscripts__lib__20260226_155403`). The `dir_key` identifies the source directory that was analyzed, allowing cleanup tools to identify and remove older runs for the same source. Each directory contains `files.txt` (analyzed files), `reference.yaml` (structured reference data), and `tasks/` + `plans/` subdirectories with extracted aitask and aiplan files. Codebrowser runs are stored under `.aitask-explain/codebrowser/` with the same structure.
