...
### Change Request 1 (2026-07-23 18:29)
- **Requested by user:** Four confirmed blocking findings from review plus one follow-up: (1) a stored `generation.inputs[].ref` in the tolerated `proj#t100` spelling (schema-valid — the inputs `ref` field is a plain string, unlike task_ref-typed fields) was preserved raw while recomputation emits canonical `proj#100`, so an unchanged repo read STALE forever with no reasons; (2) the positive artifact-handle test wrote the blob cache to the real `~/.cache` (`XDG_CACHE_HOME` not isolated); (3) `shellcheck .aitask-scripts/aitask_trail_gather.sh` exited 1 on SC1091 info while the plan requires that exact command clean; (4) the five whitelist rows violate the extension-points rule ("whitelist only helpers invoked from a skill closure") — no skill calls the helper until T3; (5) unused `plan_relpaths` variable.
- **Changes made:** (1) `_canonical_task_ref()` added; stored task-input refs, entry refs, exclusion/observation baseline refs, and `scope.topics` are all canonicalized before comparison — new regression test pins a `#t`-spelled input drifting CURRENT with zero reasons. (2) Test base class now redirects `XDG_CACHE_HOME` into the per-test temp dir (and restores it), making the handle round-trip hermetic. (3) `disable=SC1091` added to the two source directives (repo precedent: `aitask_applink.sh`); plain `shellcheck` now exits 0. (4) All five whitelist rows reverted (live + seed Claude/OpenCode/Codex surfaces); whitelisting is recorded below as a T3 deliverable. (5) Dead variable removed.
- **Files affected:** `.aitask-scripts/lib/trail_gather.py`, `.aitask-scripts/aitask_trail_gather.sh`, `tests/test_trail_gather.py`, plus reverts in `.claude/settings.local.json`, `seed/claude_settings.local.json`, `seed/opencode_config.seed.json`, `.codex/rules/default.rules`, `seed/codex_rules.default.rules`. Suite now 59 tests, all green.

### Change Request 2 (2026-07-23 18:40)
- **Requested by user:** Confirmed blocking follow-on to CR1's fix: re-spelling stored `#t` refs to canonical corrupted digest provenance in the opposite direction — a self-consistent trail whose digest was hashed over the `proj#t100` spelling would now recompute over `proj#100` and read STALE with no reasons; the regression test also had to recompute `generation.input_digest` after mutating the ref spelling.
- **Changes made:** `StoredInput` now carries both spellings: `ref` (the trail's exact stored bytes — digest reconstruction, including the residual-attribution substitution digest, reproduces them) and `canonical` (all lookups, baseline/member-set comparisons, snapshot keys, and consumer-facing DRIFT refs). The regression test recomputes the stored digest over the re-spelled records, pinning the self-consistent-trail → CURRENT contract.
- **Files affected:** `.aitask-scripts/lib/trail_gather.py`, `tests/test_trail_gather.py` (59 tests, all green).
...
- **Actual work done:** Exactly the planned surface: `lib/topic_semantics.py` (verbatim board-seam extraction; board imports it back, `TaskCard._parse_filename` delegates), `lib/trail_gather.py` (snapshot + drift verbs with every pinned contract: canonical refs, §8.1 records via `trail_schema.input_digest`, `--owner` handoff, driftable-input rule, plan-path containment, stable-read policy, version lock, trigger matrix with residual attribution, digest-independent scans over all scoped project roots, canonical DRIFT dedup/ordering), `aitask_trail_gather.sh` (wrapper with `art:` handle resolution boundary), `parse_ref()` in `cross_repo_notation.py`, and `tests/test_trail_gather.py` (59 tests, matrices A–M incl. the mandatory positive artifact-handle round-trip in a git-initialized synthetic repo).
- **Deviations from plan:** (1) Whitelist rows (plan decision 8) were REVERTED in review: `aidocs/framework/aitasks_extension_points.md` pins that helpers are whitelisted only when a skill closure invokes them — no skill calls this helper until T3, so the five rows are a **T3 deliverable** (see Notes for sibling tasks). (2) Two review rounds refined digest provenance beyond the plan: stored input refs keep their exact stored spelling for every digest reconstruction (`StoredInput.ref`) while a separate canonical form drives lookups/comparisons (`StoredInput.canonical`) — re-spelling in either direction fabricated STALE (see Post-Review Changes 1–2).
- **Issues encountered:** The wrapper's EXIT-trap cleanup used a `[[ -n ... ]] &&` short-circuit whose false branch poisoned the script's exit code to 1 under `set -euo pipefail` — rewritten as an `if`. `cmd_get --out` in `aitask_artifact.sh` prints `Wrote <path>` to stdout, which would corrupt the drift protocol stream; the wrapper redirects the get call's stdout to stderr and the integration test pins the complete stdout byte-exactly. The default-argument binding `out=sys.stdout` defeated `redirect_stdout` in tests — switched to late binding.
- **Key decisions:**
  - `premise_invalidated` is excluded from `GATHERER_DRIFT_CODES` (refresh-agent-authored; RFC §7.5 anti-fabrication). The task AC was amended pre-coding (step 0 commit) — schema enum untouched.
  - Version lock instead of a runtime compatibility mapping: a `NORMALIZATION_VERSION` bump must ship with a `schema_version` bump (matrix-M tripwire); old-schema trails read `ERROR:invalid_trail`, never false STALE.
  - Residual attribution: substitution digest (reconstructed old task records from entry snapshots + live content records) runs only when zero content transitions were already attributed; undecidable remainders get one conservative `other` naming the candidates — never silently dropped.
  - Drift verdicts are fail-closed: undriftable input kinds, unresolvable projects, path-escaping plan refs, and unstable scans all yield staged `ERROR:` lines and no CURRENT/STALE.
  - Digest reconstruction reproduces the trail's exact stored ref spellings; canonical spellings are comparison-only (Post-Review Change 2).
- **Upstream defects identified:** None
- **Notes for sibling tasks:**
  - **T3 (skill) MUST add the five whitelist rows** for `./.aitask-scripts/aitask_trail_gather.sh` when the skill first invokes it: `.claude/settings.local.json`, `seed/claude_settings.local.json`, `seed/opencode_config.seed.json`, `.codex/rules/default.rules`, `seed/codex_rules.default.rules` (rows mirror `aitask_work_report_gather.sh`; see the 7-touchpoint checklist in `aidocs/framework/aitasks_extension_points.md`).
  - T3 consumes the line protocol pinned in the `trail_gather.py` docstring. `snapshot` gives SCOPE/OWNER/MEMBER/INPUT/DIGEST; the trail's `generation.inputs` = the INPUT (kind, ref) pairs; `generation.input_digest` = the DIGEST line. Multi-topic without `--owner` emits `OWNER:none` — T3 must obtain an explicit user owner choice before `ait artifact create` (or pass `--owner`).
  - T3 authors `premise_invalidated` during refresh; the helper never emits it. The RFC §8.2 wording listing it among the drift check's codes should be synced by T6 (aidocs current-state rule).
  - Entry `snapshot`s are the drift anchor: T3 should populate status + depends + gates_pending (the gatherer's INPUT task fields) for every entry — incomplete snapshots degrade residual attribution to a conservative `other`.
  - Trails must reference the exact canonical refs the gatherer emits; hand-edited ref spellings are tolerated only if the digest was hashed over them (self-consistency).
  - The wrapper resolves `art:` handles; the board (T4) can call `drift --trail art:<handle>` directly and parse the first stdout token (CURRENT/STALE/ERROR:*).
