...
- **Actual work done:** Exactly the planned surface. New skill: `.claude/skills/aitask-trail/SKILL.md.j2` (create/refresh/show flows with all pinned contracts: latency rule, ad-hoc→task-scope mapping, `OWNER:none` explicit-owner handoff, propose-and-confirm scope expansion with executable re-gather rules, two ⚠️ NON-SKIPPABLE confirm gates, slug collision re-prompt, mandatory pre-write `drift --trail <tmpfile>` validation, stale-base `versions` re-read guard, headless-profile write refusal as the sole profile conditional) + the three stubs (`.claude/skills/aitask-trail/SKILL.md`, `.agents/skills/aitask-trail/SKILL.md`, `.opencode/commands/aitask-trail.md`). Codeagent `trail` operation: `SUPPORTED_OPERATIONS`, three per-agent invoke arms, whitespace guard extended to `work-report|trail`, help text, `_FRESH_WINDOW_OPERATIONS`, `.defaults` `"trail": "claudecode/opus4_8"` (seed + live), `verified["trail"]` mirroring `explain` in all 6 models files. Whitelists: `aitask_trail_gather.sh` + `aitask_artifact.sh` rows on all five surfaces (T2's deferred deliverable; `audit-helper-whitelist` zero MISSING for both). Tests: `test_skill_render_aitask_trail.sh` (52 asserts), `test_trail_skill_contract.sh` (60 asserts, per-golden prose-contract markers a–k, failure-path proven by a mutated-golden negative control), `test_codeagent_trail.sh` (27 asserts, t1162_2 construction-spy pattern), goldens ×3 profiles.
- **Deviations from plan:** (1) Post-review (see Change Request 1): refresh membership/ownership reconstruction was hardened — `task`/`ad_hoc` refresh replays the stored `generation.inputs` member set and every refresh re-snapshot passes the stored `--owner`; create-flow expansion with individual tasks reclassifies the trail to `ad_hoc` with a recorded transition. (2) `shellcheck aitask_codeagent.sh` had pre-existing SC1091 info-level exits (present at HEAD); fixed in-task with the repo-precedent `disable=SC1091` source-directive suffixes so the plan's "shellcheck clean" verification is real.
- **Issues encountered:** Contract-marker greps initially spanned wrapped prose lines (two markers hit line breaks) — shortened to single-line literals. The E2E plumbing smoke ran hermetically in a scratch git repo (with `XDG_CACHE_HOME` isolated) instead of a live throwaway task, mutating nothing in the real repo; it exercised the full chain: authored-per-skill-rules JSON → drift CURRENT → create `HANDLE:` → `ls` → collision refusal → member status flip → drift STALE `status_changed` via `art:` handle → v2 validate → stale-base versions compare → update → versions `*` on v2 → post-refresh drift CURRENT.
- **Key decisions:**
  - Ad-hoc scope maps to the gatherer's task scope (selected-member inputs + validated `--owner`); the trail records `scope.kind: "ad_hoc"` + `selection_note`. Sound because drift reads only `scope.topics`, never `scope.kind`. A dedicated `ad_hoc` gatherer mode was rejected (would duplicate task-scope semantics and reopen the pinned T2 protocol).
  - `--show <handle>` implemented per the task AC (the RFC's §3 matrix lacks it; J7 inspection is board-side) — user-confirmed at planning. T6 should note the CLI show path when writing docs.
  - `trail` defaults to the heavy class (`claudecode/opus4_8`) — analysis depth is the product; user-confirmed at planning.
  - Skill bash calls use full helper paths (`./.aitask-scripts/aitask_artifact.sh`, not `ait artifact`) because the codex whitelist surface has no `./ait` dispatcher shapes and the audit matcher is helper-path based.
  - Pre-write validation reuses the whitelisted gatherer wrapper (`drift --trail <tmpfile>`) as the schema+digest validator, avoiding new python whitelist rows.
- **Upstream defects identified:**
  - `aitasks/metadata/models_claudecode.json` (live task-data) — `opus4_8` carries a stats-accumulated `verified["work-report"]: 100` with no `explain` key, so `tests/test_codeagent_work_report.sh` Test 7 (t1162_2 parity rule: work-report mirrors explain, absent where explain absent) exits 1 at HEAD, before this task. Either the parity rule must tolerate live verifiedstats-accumulated scores or the accumulator must not write scores for operations whose parity partner is absent.
- **Notes for sibling tasks:**
  - T4 (board By-Trail view, t1210_4): the `trail` operation is registered and in `_FRESH_WINDOW_OPERATIONS`; launch via `[CODEAGENT_SCRIPT, "invoke", "trail", *args]` with whitespace-free args (the guard refuses otherwise). Trail discovery = frontmatter `artifacts:` scan for `kind: implementation_trail`; drift badge = first stdout token of `aitask_trail_gather.sh drift --trail art:<id>`.
  - T6 (docs, t1210_6): document the `--show <handle>` CLI path (task-AC addition not in RFC §3) and sync RFC §8.2's `premise_invalidated` wording (agent-authored, never helper-emitted — T2 note stands).
  - The skill's prose contracts are pinned per-golden in `tests/test_trail_skill_contract.sh`; any `.md.j2` edit needs goldens regenerated in the same commit or the render/contract suites go red.

## Post-Review Changes

### Change Request 1 (2026-07-24 13:15)
- **Requested by user:** Three confirmed findings from review: (1) BLOCKING — the refresh procedure rebuilt a `scope.kind: task` trail from only the initiating task/owner, so members added via the create-flow scope expansion (which re-snapshots a wider id set) would silently vanish on the next refresh; (2) BLOCKING — the refresh snapshot reconstruction never passed `--owner <loaded owner>`, so multi-topic / multi-id ad-hoc re-snapshots would emit `OWNER:none` with no defined preservation of the existing task-owned artifact linkage; (3) FOLLOW-UP — `bash tests/test_codeagent_work_report.sh` exits 1 on a pre-existing baseline defect (live `models_claudecode.json` `opus4_8` carries a stats-accumulated `verified["work-report"]: 100` with no `explain` key, violating that suite's parity rule; verified present at HEAD before this task), so the planned "full battery green" claim must be attributed, not asserted.
- **Changes made:** (1) Step 2c now pins the executable expansion re-gather: adding a topic → `--scope multi_topic` (kind `multi_topic`); adding individual tasks to a task/topic trail → `--scope task <all member ids>` with the trail reclassified `scope.kind: "ad_hoc"` + a `selection_note` recording the widened-from transition; Step 3.3 replays `task`/`ad_hoc` scopes from the stored `generation.inputs` task_file refs (complete member set, never just the initiating task). (2) Step 3.3 now REQUIRES `--owner <loaded owner local id>` on every refresh re-snapshot and states the new version's `owner` is copied unchanged (refresh never re-opens ownership). Both contracts pinned as new markers (j)/(k) in `tests/test_trail_skill_contract.sh` (60 asserts); goldens regenerated in the same change. (3) Recorded as a pre-existing upstream defect in Final Implementation Notes / Upstream defects for follow-up task creation — not repaired in this task.
- **Files affected:** `.claude/skills/aitask-trail/SKILL.md.j2`, `tests/golden/skills/aitask-trail/SKILL-{default,fast,remote}-claude.md`, `tests/test_trail_skill_contract.sh`, `aiplans/p1210/p1210_3_aitask_trail_skill.md`.
