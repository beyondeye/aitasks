...
### Change Request 1 (2026-07-20 00:30)
- **Requested by user:** A live shadow review (Codex agent, gpt-5.6-sol) of this very implementation emitted a concern block that minimonitor failed to parse — diagnose why; also address the shadow's own finding (dangling reference to an untracked aidocs file).
- **Changes made:** Diagnosed the parse failure (reproduced against the real pane capture: `has_concern_block: False`, 0 concerns). Root cause: the Codex TUI markdown renderer hard-wraps long output rows with literal newlines that `tmux capture-pane -J` cannot rejoin, and the wrap landed *inside* the `[priority | region]` marker (48-char full-path region), so the item regex never matched and the whole item was silently dropped. Fixes applied: (a) short-region rule (≤ ~30 chars, `basename.ext:LINE`, full path in body) added to both `impl-challenge.md` (rules + example) and `concern-format.md` (single source of truth), with the rendered-row rationale; (b) `aidocs/codeagents/claudecode_builtin_prompts.md` added to the commit scope so the catalog's reference resolves on a clean checkout; (c) the parser fragility recorded as an upstream defect (see Final Implementation Notes) — user confirmed the producer-side rule is a mitigation only and the parser hardening should be a standalone follow-up.
- **Files affected:** `.claude/skills/aitask-shadow/impl-challenge.md`, `.claude/skills/aitask-shadow/concern-format.md`, commit scope (+`aidocs/codeagents/claudecode_builtin_prompts.md`).

## Final Implementation Notes

- **Actual work done:** Implemented the four-tier shadow implementation review exactly per plan: new shared catalog `.claude/skills/aitask-shadow/impl-review-angles.md` (correctness angles A–E, cleanup angles + precedence, legacy axes S0/S1/S2 one-to-one from the pre-change file, anti-drop rule, verdict ladder + recall addendum, gap-sweep list, angle-independent disposition rubric, partition-before-cap ordering/overflow/disclosure rules); rewrote `impl-challenge.md` (tier selection with free-text detection + 4-option chooser + deterministic two-stage Codex adaptation, angle-scoping guard rails, angle-activation table, Quick/Basic/Standard/Deep tier definitions, disposition+verdict findings presentation, frozen concern-block wire format with partition ordering, minimonitor UX-boundary note); updated SKILL.md Step 3 routing (tier phrases; unqualified "adversarial review" → basic); amended the task AC (verify ladder scoped to Standard+Deep, Basic recorded as legacy-compatibility exception) before skill edits; added the focused disposition/verdict body-trailer round-trip test to `tests/test_concern_parser.py`.
- **Deviations from plan:** Two post-review additions prompted by a live shadow review of this implementation (see Change Request 1): the short-region producer rule in impl-challenge.md + concern-format.md, and committing `aidocs/codeagents/claudecode_builtin_prompts.md` (the extracted-prompt reference the catalog cites) with the feature. Both were user-approved; everything else landed as planned.
- **Issues encountered:** A live Codex shadow's concern block failed minimonitor parsing during the review of this very task — full diagnosis and mitigation in Change Request 1; the structural fix is deferred to the upstream-defect follow-up below.
- **Key decisions:** Basic preserved as the one-to-one legacy compatibility tier (S0/S1/S2, single full-context pass, no verify, no cap) with unqualified "adversarial review" routing to it; Standard is the recommended tier; disposition classified by reachable impact vs task obligations (never by angle or verdict); caps partition-aware with a deterministic blocking-overflow rule and mandatory omission disclosure; concern-block wire format frozen (disposition/verdict as body free text) so the future concern-format redesign has one seam; shadow's honesty rules override the extracted min-findings floors in every tier.
- **Upstream defects identified:** `.aitask-scripts/monitor/concern_parser.py:57 — the item regex requires the complete [priority | region] bracket on one captured row; agent-TUI renderers (observed: Codex CLI markdown renderer at ~55 cols) hard-wrap long rows with literal newlines that the -J wrap-join cannot rejoin, so a bracket split mid-region silently drops the item and the auto-offer never fires. Foolproof fix: bounded wrap-tolerant marker matching (join at most 2-3 following rows until the bracket closes before matching, preserving the wrap-collision hardening), with the live Codex capture as a test fixture. The short-region producer rule added in this task mitigates but cannot enforce.`
