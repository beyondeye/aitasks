...
5. **Verdict engine:** tri-state `FRESH / ASK_STALE / SKIP`, line protocol modeled on `aitask_verification_stale.sh` (`BASELINE / CHECKED / FINGERPRINT / FILES / CHANGED / DELETED / UNKNOWN / DISPLAY / DECISION`), always exit 0 for content states, die on CLI misuse. The new `CHECKED:<sha>` line records the exact revision the evidence was evaluated against (HEAD at check time); **every later baseline advance writes that recorded sha, never write-time HEAD** — the check result and the lock-and-write transaction are separated by Step 4, and HEAD may move in between; advancing to write-time HEAD would silently cover commits the user was never shown. Commits after the checked revision stay uncovered and surface at the next pick. The revision is only half the binding: **`FINGERPRINT:<digest>` binds the metadata inputs the verdict depended on** — a digest over the canonicalized input tuple (baseline source + sha, scope tier, sorted scope path list, resolved origin ids) — because `CHECKED:` alone cannot detect a concurrent mutation of `file_references:`, the follow-up origin fields, or the stored baseline itself between prompt and write (another session or a sync merge can change them; the `active_gates_digest` claim-time pattern is the precedent). **After the Step 4 lock is acquired and before any baseline write, the producer is re-run and its fingerprint compared to the prompted one: equal → the write proceeds; different → the prompted confirmation is void — present the fresh evidence and require a fresh decision before advancing.** Framework law preserved: **UNKNOWN drives the verdict** (one evidence list), **empty scope → SKIP not FRESH**, `%`-then-`|` injective encoding, `:(literal)` pathspec guard, probes against committed trees (dirty worktree invisible by construction).
...
7. **Interaction:** new **Step 3 Check 6** in task-workflow (fires on every entry path: pick, board launch, explore; Ready tasks only — resume paths skip it). `FRESH`/`SKIP` → silent. `ASK_STALE` → one NON-SKIPPABLE AskUserQuestion with the evidence inside the widget text, four options:
   - *Proceed — premise still valid*: advance `premise_baseline` to the **checked revision** (the `CHECKED:` sha, not write-time HEAD); write deferred until after the Step 4 lock claim **and gated on the post-lock fingerprint re-check** (invariant 5): a changed scope/origin fingerprint voids this answer and re-prompts. Transaction: re-check fingerprint → decide → write → advance baseline **last** → commit.
...
2. **Create the implementation parent task + sequential children** (via the Batch Task Creation Procedure, `--followup-of 1561` so anchor resolves to root 1538). Children auto-depend on siblings; each description self-contained per Child Task Documentation Requirements:
   1. **Core engine**: `lib/task_premise.py` (pure; generalized `baseline_for`/`check`; `PURE_MODULES` entry) + `aitask_premise_stale.sh` producer + protocol tests. **Owns these verification cases, each with its pinned expected outcome**: clean → `FRESH`; changed scope → `ASK_STALE` with `CHANGED:`; uncheckable path → `UNKNOWN:` drives `ASK_STALE`; metadata absent (no baseline, no scope) → silent `SKIP`; metadata present in every tier combination (stored/computed baseline × curated/derived scope); **history rewrite** — stored baseline unreachable from the checked revision (`merge-base --is-ancestor` fails, and the unresolvable-rev 128 case) → `SKIP`, never an error; **dirty worktree** — an uncommitted edit to a scope file produces no `CHANGED:` and flips no verdict (probes read committed trees only; mirrors `test_dirty_worktree_is_not_a_change`); negative controls (forced-failure per scope tier); **fingerprint canonicalization** — identical inputs yield an identical `FINGERPRINT:` digest, and a change to any element of the input tuple (baseline source/sha, scope tier, any scope path, origin ids) yields a different one; and the no-go shape if the pre-phase takes it (stored baseline + derived scope → verdict; no stored baseline → `SKIP`).
   ...
   4. **Workflow integration**: Step 3 Check 6 + new procedure file `.claude/skills/task-workflow/premise-staleness.md` (one-line caller per authoring conventions), NON-SKIPPABLE prompt marking, `aitask_skill_verify.sh` + per-profile rerender + goldens in the same commit. **Owns the UI/workflow verification**: the end-to-end exercise (seed → change a scope file → prompt fires → dismiss → baseline advanced → no re-fire); the TOCTOU pins — both axes: a commit landing between check and write (advanced baseline equals the `CHECKED:` sha, not the newer HEAD) and a **concurrent scope-metadata mutation** (`file_references:` / origin fields / stored baseline edited between prompt and post-lock write — the post-lock fingerprint re-check detects the mismatch, voids the stale confirmation, and re-prompts with the fresh evidence; the baseline is never advanced on the stale answer); the replan path (baseline unchanged at dismissal, advanced only after plan approval).
