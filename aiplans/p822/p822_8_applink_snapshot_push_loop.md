...
- **Actual work done:** Implemented Stage 1 of the applink binary data plane exactly as planned.
  - `applink/content.py` (NEW): ad-hoc SGR/OSC8 state machine (`parse_sgr_line`, `snapshot_to_rows`) → styled-span rows; `char_width` (combining/zero-width → 0, EAW W/F → 2); `encode_keyframe`/`encode_cursor`/`encode_dim` (1 raw type byte + msgpack array of remaining fields, msgpack imported lazily); `Subscription` pure state (panes, clamped cadences, focus, per-pane frame_id/hash/dims, force set) + cadence floors.
  - `applink/pusher.py` (NEW): per-connection `PushScheduler` with a `_run_once` emit pass driven by a `_loop` timer; emits `dim` on resize and `keyframe` on change/forced/keyframe-interval; `pane_status` JSON push at idle cadence (incl. derived `task_id`); back-pressure high-water guard; deterministic `stop()` (cancel + await).
  - `monitor/monitor_core.py`: `capture_cursor_async` (display-message; block-cursor style 0).
  - `applink/router.py`: promoted `subscribe`/`request_keyframe` from deferred → implemented; threaded `conn` into `_dispatch`; `focus` now also raises subscription cadence; `ConnState.subscription`.
  - `applink/server.py`: per-connection `PushScheduler` lifecycle (`_ensure_pusher`, wake on active subscription, `stop()` in the connection `finally`).
  - Profiles: `subscribe`/`request_keyframe` added to all 3 YAMLs + `profiles.py` `DEFAULT_ALLOWED`. Deps: `msgpack>=1,<2` in `aitask_setup.sh` + launcher probe. Doc: corrected the `focus`/read_only sentence in `monitor_port_design.md`.
- **Deviations from plan:** None of substance. (1) The encoder lives in the applink package, not `monitor_core` — a confirmed key decision to keep msgpack off the board/monitor TUI import path. (2) `focus` stays `monitor_control+` (cadence via `subscribe` for read_only) — confirmed decision; standalone `cursor` (0x04) frames are encoded+tested but the scheduler folds cursor into keyframes (documented Stage-1 limitation so idle panes cost zero binary bytes).
- **Issues encountered:** (a) `msgpack` was not in the existing venv — installed `msgpack 1.2.0` to run the tests locally; `ait setup` now installs it. (b) The `osc8` sidecar uses **int** span-offset keys; `msgpack.unpackb` defaults to `strict_map_key=True` and rejects them — decoders must pass `strict_map_key=False` (noted for mobile). (c) Two test typos (a stray char in a BEL-OSC8 string; the strict-map-key default) fixed during the test run.
- **Key decisions:** Wire framing = 1 raw type byte + msgpack array of the *remaining* fields (NOT type as msgpack array[0]) — matches content_transport.md's "1-byte tag + remainder is MessagePack" prose. Cadence is a `subscribe` concern, `focus` a control concern. Scheduler split into `_run_once`/`_loop` for deterministic async unit testing.
- **Upstream defects identified:** None.
- **Notes for sibling tasks:**
  - **t822_9 (delta) / t822_10 (append):** extend `applink/content.py` — `Subscription.PaneState` already tracks `frame_id`/`last_hash`/`last_dims`; the deltifier (row hashing + changed-row collection) and append fast-path slot into `pusher.PushScheduler._push_pane`'s keyframe-on-change branch. `FRAME_DELTA`(0x02)/`FRAME_APPEND`(0x03) tags are reserved in `content.py`.
  - **Mobile (cross-repo `aitasks_mobile`):** decode framing = `frame[0]` is the raw type tag, `msgpack.unpackb(frame[1:])` is the field array (configure the decoder to **allow int map keys** for the `osc8` sidecar). `pane_status` is a `kind:"push"` JSON **text** frame carrying idle/awaiting/identity/`task_id`; full task title/status stay behind the `task_detail` verb.
  - **t822_12 (permissions doc sync):** `subscribe`/`request_keyframe` are now real gated verbs in all 3 profiles (gated like `snapshot`); `focus` remains `monitor_control+`.
  - **t985 (security hardening) / applink_dataplane_limits_hardening (this task's "after" mitigation):** the data plane currently clamps cadences to floors but does NOT cap subscribed-pane count, msgpack frame size, or guard decode bombs — that hardening is the queued follow-up.
