---
Task: t1405_2_promote_testing_and_verification_cluster.md
...
# p1405_2 — Promote the testing & verification cluster

## Context

Child 2 of t1405. Promotes ~25 memories into `aidocs/framework/testing_conventions.md`.
...
3. **Execute (gate Phase 3).** Per ruling: journal to
   `.aitask-memtriage/t1405_2.tsv` → apply the doc edit and the deletion and
   the `MEMORY.md` line removal → flip `state` to `done`.
4. **The negative-control family is a MERGE, not six promotions.** Collapse the
   six near-duplicates into one progression: harness-can-fail → test-
   discriminates → one-mutation-per-test → restore-without-`git checkout`.
   Record the source → merged `doc#heading` mapping for all six.
5. **The known-stale exemplar.** `project_python_runner_k_filter_runs_nothing`
   motivated this whole task. Promote only the still-true half (`-k` runs zero
   tests on the `unittest` fallback; a positional path *widens* the run), drop
   the pytest-absent premise, and check what CLAUDE.md's Testing section
   already documents so the rule is not written twice.
