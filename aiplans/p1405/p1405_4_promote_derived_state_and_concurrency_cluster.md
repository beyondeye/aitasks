---
Task: t1405_4_promote_derived_state_and_concurrency_cluster.md
...
# p1405_4 — Promote the derived-state & concurrency cluster

## Context

Child 4 of t1405. Promotes ~21 memories into `aidocs/framework/state_and_concurrency_conventions.md` (NEW).
...
3. **Execute (gate Phase 3).** Per ruling: journal to
   `.aitask-memtriage/t1405_4.tsv` → apply the doc edit and the deletion and
   the `MEMORY.md` line removal → flip `state` to `done`.
4. **Create the doc and wire it into all five surfaces.** H1 + scope preamble
   pointing at sibling docs, then the rule sections. Then add it to
   `aidocs/framework/README.md`'s **entrypoint-advertised** list and give it a
   trigger in `CLAUDE.md` and in each of the three out-of-marker appendices
   (`AGENTS.md`, `.codex/instructions.md`, `.opencode/instructions.md`) —
   after the closing `<<<aitasks` marker only, never in the seed.
   `tests/test_aidocs_pointer_parity.sh` fails until all four carry it.
5. **Cross-reference `testing_conventions.md` both ways** — it owns testing a
   threading/asyncio migration; this doc owns designing the state itself.
