...
Sibling Tasks: aitasks/t688/t688_2_surface_textual_upgrade_in_setup.md
Archived Sibling Plans: aiplans/archived/p688/p688_1_fix_select_set_options_crash_textual_8_0.md
Base branch: main
plan_verified:
  - claudecode/opus4_7_1m @ 2026-04-28 12:20
---
...
Three-piece change: a new seed asset, an `install.sh` hook that relocates it to a stable runtime location (because `install.sh` deletes `seed/` at line 1030), and a setup-time helper that prompts the user and copies the template into `$HOME` on consent.

The `cp`-based pattern matches `install_seed_aitask_scripts_gitignore` (`install.sh:273`) — the closest precedent for a fixed, non-user-customizable framework asset.
...
- `.aitask-scripts/aitask_setup.sh` — add `setup_starter_tmux_conf()` reading from `$SCRIPT_DIR/templates/tmux.conf`. Slot into `main()` immediately after `setup_tmux_default_session` (current line 3043), before `setup_userconfig`.

## Reference Files for Patterns

- `install.sh:273` — `install_seed_aitask_scripts_gitignore`: closest precedent for `cp`-based fixed-asset install (no user merge needed).
...
- `install.sh:976–1030` — main install sequence (where to wire the new call).
- `aitask_setup.sh:7` — `SCRIPT_DIR` definition (resolves to `.aitask-scripts/`).
- `aitask_setup.sh:555` — `install_global_shim`: precedent for writing under `$HOME` with `mkdir -p`.
- `aitask_setup.sh:2889` — `setup_tmux_default_session`: adjacent function with `if [[ -t 0 ]]; then read -r ...` interactive pattern.
- `aitask_setup.sh:3043` — exact insertion point inside `main()`.
...
Wire into `main()` between `install_seed_project_config` (line 988) and `install_seed_reviewtypes` (line 991):
...
Insert immediately after `setup_tmux_default_session()` (after the closing `}` at line 2935):
...
Wire into `main()` between `setup_tmux_default_session` (line 3043) and `setup_userconfig` (line 3046):
