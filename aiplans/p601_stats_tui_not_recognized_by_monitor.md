...
- **Actual work done:** Implemented as planned. Created `.aitask-scripts/lib/tui_registry.py` (`TUI_REGISTRY`, `TUI_NAMES`, `BRAINSTORM_PREFIX`, `switcher_tuis`). Rewired `tmux_monitor.py` (`DEFAULT_TUI_NAMES = TUI_NAMES`, `BRAINSTORM_PREFIX` used in `classify_pane`), `agent_launch_utils.py` (top-level `from tui_registry import TUI_NAMES as _DEFAULT_TUI_NAMES`), and `tui_switcher.py` (`KNOWN_TUIS = switcher_tuis()` plus registry-sourced `_TUI_NAMES` / `_BRAINSTORM_PREFIX`). Switched both config loaders from replace to merge semantics. Removed the stale `tui_window_names` block from `aitasks/metadata/project_config.yaml`. Updated `tests/test_git_tui_config.py` with four new tests (stats in both sets, registry coverage, merge semantics) — 16/16 tests pass. Updated `website/content/docs/tuis/monitor/reference.md` classification rule, example YAML, and config-table row to describe the merge semantics in current-state language.
- **Deviations from plan:** One correction — `aitasks/metadata/userconfig.yaml` turned out to contain only `email` and `last_used_labels`; the "tmux" block I thought lived there was actually the output of a chained `cat ... ; echo ---; cat project_config.yaml` where the `---` I typed became a false YAML-document separator in my reading. Nothing to clean up in userconfig.yaml; section 4b of the plan became a no-op.
- **Issues encountered:** None. Imports across `tmux_monitor.py` (which lives in `.aitask-scripts/monitor/`) into `tui_registry.py` (in `.aitask-scripts/lib/`) required adding a sys.path insert at the top of `tmux_monitor.py` — consistent with how `monitor_app.py` already imports other `lib/` modules.
- **Key decisions:** Kept `git` as a non-registry entry (always in `TUI_NAMES`) so the git TUI stays config-driven via `tmux.git_tui`. Kept `brainstorm` and `minimonitor` as `in_switcher=False` registry entries so they classify as TUI windows without polluting the switcher modal. Preserved backward-compatible names `DEFAULT_TUI_NAMES` (tmux_monitor) and `_TUI_NAMES` (tui_switcher) as simple aliases of the registry's `TUI_NAMES` so external imports keep working.
- **Build verification:** No `verify_build` configured in `project_config.yaml`.
