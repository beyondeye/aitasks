...
The user asked for the freeze feature to be documented **as a workflow**,
not only as TUI reference (t1705_9). This child writes the workflow page,
the framework-session concept page, the setup "Session hooks" section and
the cross-links, and adds the manual `workflows/_index.md` entry.
Current-state prose from the landed code; run `check_links.py --build`.

## Files

- **New** `website/content/docs/workflows/freeze-and-restore-agents.md`
- **New** `website/content/docs/concepts/framework-session.md`
- **Edit** `website/content/docs/workflows/_index.md` (manual list), `website/content/docs/concepts/_index.md`, `concepts/locks.md` / `agent-memory.md` (one-line pointers where per-user state is listed)
- **Edit** the setup page (`grep -rl 'ait setup' website/content/docs | head`) — "Session hooks" section
- **Edit** `website/content/docs/tuis/frozenagent/_index.md` (link back), `workflows/parallel-development.md`, `workflows/crash-recovery.md` (one-line pointers)

## Implementation steps

1. **Workflow page** — front matter like `workflows/parallel-development.md`
   (`title: "Freeze and Restore Agents"`, `linkTitle`, `weight` beside
   Crash Recovery, `description`). Sections in this order: `## When to
   freeze` (table: live / parked / frozen × process alive · captured each
   tick · output kept · how you come back · cost), `## The daily loop`
   (freeze from minimonitor with the real key, read later in the same pane
   layout, search/copy, **restore vs re-pick — re-pick is usually cheaper**
   and why, drop), `## Before shutting down` (Freeze-All; Restore-All after
   the server returns; window re-creation and `-2` suffixes), `## What
   "unverified" means` (hook-verified vs liveness acknowledgement; capture
   kept; when it happens — hooks not installed, or Codex if t1705_1 found
   no hook support), `## When something goes wrong` (agent exits / wrong
   session / binary missing → viewer back, capture intact; dead stand-in
   respawned by reconcile; windows that no longer exist; the maintenance
   tick and `aitask_frozen.sh reconcile` — cite the real invocation, there
   may be no `ait frozen` verb), `## Marks and frozen agents`, `## Limits`
   (no expiry, disk under `~/.config/aitasks/frozen/`, the capture cap and
   `frozen.capture_max_lines`).
2. **`workflows/_index.md`** — bullet in the parallel/crash-recovery group:
   `- [Freeze and Restore Agents](freeze-and-restore-agents/) — Freeze finished
   agents to free their processes while keeping their output readable in
   place, and bring them back by resume or re-pick.`
3. **Concept page** — `concepts/framework-session.md` (front matter like
   `concepts/locks.md`): the store and its file, one record per agent across
   projects, identity (`root` + window + slot; pane id/pid are location),
   the state diagram (mermaid fence — Docsy renders it; verify in the
   build), operation leases and why reconcile respects them, the
   SessionStart hook and `@aitask_record`, purge rules, env overrides. Link
   from `concepts/_index.md` and the two sibling concept pages.
4. **Setup docs** — "Session hooks" section: what `ait setup` writes into
   `.claude/settings.json` and `.codex/config.toml`, preservation of user
   hooks, idempotency, the TOML-comment loss, opt-out (verify the actual
   re-add behaviour in `aitask_setup.sh` before stating it).
5. **Cross-links** — the three one-liners listed in Files.
...
>
> **Why this matters for this task specifically.** This task PUBLISHES the
> freeze/restore workflow docs. Documenting §D's original `env`-prefix mechanism
> would ship the superseded contract to users. Document B1–B4.
