...
- **Edit** `install.sh` — `install_seed_claude_hooks()` (beside :718-729), call site (before :1341), framework-path list (:1006-1019)
- **Edit** `.aitask-scripts/aitask_setup.sh` — `ensure_agent_config_seeds` pair (:2237-2243), new `merge_claude_hooks()` (beside :2479), `setup_claude_code()` (:2530-2575) branch for `.claude/settings.json`, `setup_codex_cli()` step 3 (:2790-2802) + `toml_serialize` (:2601-2655) verification, `_ait_framework_paths()` (:3387-3404)
...
- **New tests** `tests/test_session_hook.sh`, `tests/test_session_hook_install.sh`
- **Check** `.github/workflows/release.yml:85-96` — seed file shipped
...
1. **Hook script** (`aitask_session_hook.sh`, `#!/usr/bin/env bash`, `set -uo pipefail` — **not** `-e`; every failure path must reach `exit 0`):
   ...
   read -r sid tpath cwd src < <(printf '%s' "$payload" | python3 -c '
   import json,sys
   d=json.load(sys.stdin); print(d.get("session_id",""), d.get("transcript_path",""), d.get("cwd",""), d.get("source",""))' 2>/dev/null) || exit 0
   ...
   args=(upsert --root "$root" --window "$win" --pane "$pane" --pane-pid "$ppid" --session-id "$sid" --transcript "$tpath"
   ...
   comment documents the exit-0 contract and the Codex finding.
2. **Seed files.** `seed/claude_settings.hooks.json` exactly as in the task
   file. `seed/codex_config.seed.toml` gains the `[hooks]` block t1705_1
   validated (comment above it naming the finding date).
3. **`install.sh`.** `install_seed_claude_hooks()` copies
   `seed/claude_settings.hooks.json` → `aitasks/metadata/claude_settings.hooks.json`;
   call it next to `install_seed_claude_settings` (:1322-1336); add
   `.claude/settings.json` to the framework path list (:1006-1019) with the
   sync comment.
4. **`aitask_setup.sh`.** (a) `ensure_agent_config_seeds` pair
   `"claude_settings.hooks.json:claude_settings.hooks.json"`. (b)
   ...
   other key**. (c) `setup_claude_code`: after the `settings.local.json`
   block, `.claude/settings.json` absent → `cp` seed; present →
   `merge_claude_hooks`; record via `_note_metadata_write`-style bookkeeping
   so `commit_framework_files` picks it up. (d) `setup_codex_cli` step 3:
   run `toml_serialize` on a dict containing the `[hooks]` shape and
   `tomllib`-parse the result in a test; extend the serializer if a nested
   table / array-of-tables the shape needs is not emitted. (e)
   `_ait_framework_paths()` gains `.claude/settings.json`.
5. **This repo's `.claude/settings.json`** — add the entry by hand (the
   guard hook stays first).
6. **Fallback resolver** `newest_transcript_for(root, agent_kind)` in
   `lib/agent_sessions.py`: Claude → newest `*.jsonl` under
   `~/.claude/projects/<escaped-cwd>/` (escape rule: `/` → `-`, verify
   against a real dir); Codex → newest file under `~/.codex/sessions/**`
   whose first JSON line's `cwd` equals `root`. Returns `(session_id,
   transcript_path)` or `("", "")`.
