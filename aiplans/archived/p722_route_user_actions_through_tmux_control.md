...
- **Actual work done:** `TmuxControlBackend` added to `tmux_control.py` (~155 lines): owns a daemon thread + dedicated asyncio loop driving a `TmuxControlClient`, exposes `start()/stop()/is_alive` plus `request_sync()` and `request_async()`. `TmuxMonitor` migrated from holding a raw `TmuxControlClient` to holding a `TmuxControlBackend`; `start_/close_/has_control_client()` kept their async signatures (callers unchanged) but now delegate to the backend. New `_run_tmux_subprocess` sibling of `_run_tmux_async` is the single subprocess fallback path. New `TmuxMonitor.tmux_run(args, timeout)` is the canonical sync entry point. All in-scope user-action sites in `tmux_monitor.py` (13 sites: `_discover_panes_multi`, `discover_panes`, `discover_window_panes`, `capture_pane`, `send_enter`, `send_keys`, `switch_to_pane` × 2, `find_companion_pane_id`, `kill_pane`, `kill_window`, `kill_agent_pane_smart`, `spawn_tui`), `monitor_app.py` (`_consume_focus_request`, `_clear_focus_request`, `_read_attached_session`), and `minimonitor_app.py` (`_update_own_window_info`, `_find_sibling_pane_id`, `_focus_sibling_pane`, `action_switch_to_monitor` × 4) routed through `tmux_run`.

  Tests: extended `tests/test_tmux_control.sh` with Cases 5, 6, 7, 8, 9, 11, 12 (~9 new test cases, ~360 lines). New `tests/test_tmux_run_parity.sh` exercises every migrated subcommand against subprocess in both backend-on and backend-off passes. New `tests/test_kill_agent_pane_smart.sh` covers the `kill_agent_pane_smart` heuristic end-to-end on both code paths with a monkey-patched companion sentinel.

- **Deviations from plan:**
  - **Case 10 dropped.** The planned timeout-marks-dead-client test was racy at sub-millisecond timeouts (tmux IPC frequently completes in <1ms, beating the timeout). The behavior it was meant to verify is already exercised end-to-end by Case 9 (server-kill → dead → all subsequent return `(-1, "")`).
  - **Three additional `monitor_app.py` sites identified as pre-init and left as `subprocess.run`.** The plan called out two `_detect_tmux_session()` helpers, but the audit also surfaced `monitor_app.py:on_mount` (rename-window + has-session) and `SessionRenameDialog.on_button_pressed` (rename-session) as running before `self._monitor` is constructed. Same for `minimonitor_app.py:on_mount` (display-message). All five remain on `subprocess.run` and have inline comments documenting why. This is a strictly larger out-of-bounds set than the plan originally listed.
  - **Plan's note on collapsing `minimonitor_app.py:on_mount` into `_update_own_window_info` not pursued.** The current call is racy with `_start_monitoring()` ordering and the cost of routing it through `tmux_run` is one additional subprocess hop on TUI startup — leaving it as raw subprocess is simpler and the plan explicitly authorized this fallback ("if it adds churn, leave the on_mount call as subprocess").

- **Issues encountered:**
  - **Case 11 broke on `#!/usr/bin/env bash` python wrapper.** The user has `~/.aitask/bin/python3` as a one-line bash wrapper around the venv interpreter (`exec "$HOME/.aitask/venv/bin/python" "$@"`). When the test set `PATH="$NO_TMUX_DIR"` to simulate "tmux missing", the wrapper's shebang (`/usr/bin/env bash`) failed because `bash` was no longer on PATH. Fixed by resolving to the *real* interpreter via `python3 -c 'import sys; print(sys.executable)'` — that way the heredoc runs against the venv binary directly and the restricted PATH only affects what the test child process can find.
  - **Parity test exposed a real divergence on error paths.** Tmux's control mode emits the error body inside the `%error` reply, which the control client returns as stdout; raw `subprocess.run` writes errors to stderr (which the client discards). For commands that fail (rc != 0) the two paths produce different stdout. Every migrated caller only inspects stdout when rc == 0, so this is harmless and within contract. The parity test's assertion was relaxed to: rc must always match; stdout must match exactly only when rc == 0. Documented in the test's `assert_parity` docstring.

- **Key decisions:**
  - **Backend lifecycle:** `start()` is sync (the plan's pseudocode), spawns the bg thread, starts the loop, then schedules `client.start()` on it via `run_coroutine_threadsafe(...).result(timeout)`. If client.start fails, `stop()` is called inline so we never leak the thread on a failed start. Idempotent: `start()` while running returns current `is_alive` and reuses the same thread. Restartable: after `stop()`, a fresh `start()` succeeds.
  - **`start_control_client()` kept `async def`** so the four existing call sites in `monitor_app.py` / `minimonitor_app.py` (each `await self._monitor.start_control_client()`) continue to compile unchanged. The body is synchronous (the bg thread does the asyncio work); awaiting a synchronously-completing coroutine yields once and resumes — no correctness impact.
  - **Subprocess fallback always available.** `tmux_run` and `_tmux_async` both fall through to `_run_tmux_subprocess` / `_run_tmux_async` when the backend isn't running or returns `-1`. Behavior is identical to t719's fallback: degraded mode keeps working with no observable change to the caller.

- **Upstream defects identified:** None.

  - The pre-existing `# shellcheck disable=SC2031` annotation on Case 4 in `tests/test_tmux_control.sh:148` doesn't suppress the new SC2030 info that fires on the same line (existed before this task; out of scope to chase). The new cases I added use `# shellcheck disable=SC2030,SC2031` so they're clean.

- **Notes for sibling tasks:** Not applicable — t722 is a standalone parent task.
