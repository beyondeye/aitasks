...
This task creates the `/aitask-review` Claude Code skill — a Claude-driven code review workflow that uses the review guides infrastructure (from t129_3, now at `aireviewguides/*.md`) to perform targeted code reviews. The skill loads review guide files, auto-detects the project environment, lets the user select modes and target areas, performs the review, presents findings, and creates tasks from selected issues — then optionally hands off to the shared `task-workflow` pipeline.
...
**1b. Review Guide Selection** — Load modes from `aireviewguides/`, auto-detect environment, sort by relevance, paginated multiSelect.
