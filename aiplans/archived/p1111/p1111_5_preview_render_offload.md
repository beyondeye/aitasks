...
### Change Request 1 (2026-07-05 — pre-implementation plan review)
- **Requested by user:** Five concerns on the plan — (1) deferred scroll restore not re-guarded at execution time; (2) `run_worker(exclusive)` cancellation overstated as a perf mitigation (`to_thread` is uncancellable); (3) transient header/body inconsistency on a pane switch; (4) refined plan must be persisted to `aiplans/` before coding; (5) no scheduling-path test on the real sync entry point.
- **Changes made:** (1) deferred scroll callbacks wrapped in a `_guarded` re-check of gen + focused pane; (2) added a pre-offload gen check and reworded the contract so `exclusive` is a best-effort CPU reducer, not the correctness mechanism; (3) synchronous `…` placeholder on cross-pane switch (skipped on same-pane re-render); (4) plan externalized + `plan_verified` appended before implementation; (5) added test (g) driving the real `_update_content_preview`.
- **Files affected:** plan only (design), then implemented in `monitor_app.py` + `tests/test_monitor_preview_offload.py`.

### Change Request 2 (2026-07-05 — post-implementation review)
- **Requested by user:** `_preview_render_gen` was bumped only when scheduling a render, so the empty, no-focus/missing-snapshot, and frozen/paused branches failed to invalidate an already-running same-pane render — a slow in-flight render for pane A could later overwrite the (empty)/prompt/frozen view (and schedule stale scroll restore).
- **Changes made:** Moved the `_preview_render_gen` bump to the **top** of `_update_content_preview` (every re-entry supersedes a pending render); the render branch reuses that token. Added three gated regression tests (`test_{empty,no_focus,frozen}_branch_discards_inflight_render`) — the no-focus case retains focus so only the token bump can discard, and the empty case includes a negative control proving the bump is load-bearing.
- **Files affected:** `.aitask-scripts/monitor/monitor_app.py`, `tests/test_monitor_preview_offload.py`.

## Final Implementation Notes
- **Actual work done:** Offloaded the preview render (`_ansi_to_rich_text`) off the UI thread. `monitor_app.py`: `_update_content_preview` stays sync, bumps an app-owned `_preview_render_gen` up front (every entry supersedes a pending render), sets cheap loop-side state synchronously, and in the active non-empty branch schedules `_apply_preview_render` via `run_worker(exclusive=True, group="preview")` after a synchronous cross-pane `…` placeholder. New `_apply_preview_render` worker runs the pure render through the t1111_4 `_run_offloaded` seam and applies `preview.update` + scroll restore on the loop, guarded by the generation token (pre-launch, post-await, and inside each deferred scroll callback) + a focus-identity check, fail-closed to raw `Text`. `_ansi_to_rich_text` in `monitor_shared.py` was already pure — no split needed; unchanged.
- **Deviations from plan:** Two design refinements vs. the original task note (both folded into the approved plan before coding): dropped the `@work` suggestion in favor of the file's existing `run_worker` idiom, and used a **new app-owned `_preview_render_gen`** for render supersession rather than reusing the monitor-owned capture generation (which is bumped every 0.3s/3s and would spuriously discard renders).
- **Issues encountered:** Adding the render worker perturbed event-loop timing enough to surface a latent fragility in the sibling `test_monitor_focus_switch::test_structural_refresh_reconciles_selected_state_and_mapping` under full-suite load (its deferred `_restore_focus` reconcile didn't settle within a single `pilot.pause()`). Fixed the test fixture: added `_run_offloaded` to its `_FakeMonitor` (the fake now implements the seam the production path invokes) and drained workers + deferred callbacks before asserting. Confirmed via a from-baseline run that the remaining 2 `tui_switcher_agent_launch` failures + 1 loader error are pre-existing aggregate-isolation artifacts, present without this change.
- **Key decisions:** Generation token is **app-owned** (a render is not a capture); supersession correctness comes from the generation guard, with `run_worker(exclusive)` + a pre-offload check as best-effort CPU reducers (the `to_thread` work is not cancellable). The gen bump lives at the top of `_update_content_preview` so non-render branches also invalidate stale renders.
- **Upstream defects identified:** None.
- **Notes for sibling tasks (t1111_6):** This offload is now live for the focused-pane preview render; behavioral verification of "switching to/among ≥1 active `agent-*` window stays instant" is t1111_6's to confirm (py-spy was not required/available). The offload seam is `TmuxMonitor._run_offloaded` (t1111_4). Supersession is deterministic via `_preview_render_gen`; any future preview-touching code must bump it (already done centrally in `_update_content_preview`).
