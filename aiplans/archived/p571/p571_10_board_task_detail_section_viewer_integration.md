...
- **Issues encountered:**
  ...
  - **Import-order bug found during manual testing (fixed in follow-up commit `4756cc39`):** the first implementation imported `from brainstorm.brainstorm_sections import parse_sections` *before* `from section_viewer import SectionMinimap`, which failed at runtime with `ModuleNotFoundError: No module named 'brainstorm'`. Root cause: `section_viewer.py` self-inserts `parent.parent` (`.aitask-scripts/`) into `sys.path` on import, so `brainstorm.*` is only reachable AFTER `section_viewer` has been imported. The board's own sys.path insert at line 12 only covers `lib/`, not `parent.parent`. Fix: use `section_viewer`'s convenience re-export (`from section_viewer import SectionMinimap, parse_sections`) as already documented in t571_5's Final Implementation Notes. **Sibling tasks t571_8 and t571_9 must follow this pattern** — their plans have been updated.
