...
- **Actual work done:** as planned (steps 1-7 + post-phase skill_contract_test).
  - `.claude/skills/aitask-brainstorm-discuss/`: stub `SKILL.md` (resolver key `brainstorm-discuss`), `SKILL.md.j2` (What this is → Arguments → Step 0 fast start → Context on demand → Capabilities [shortcode grammar, inline, structured] → Guardrail), and four Jinja-free procedures `discuss-audience.md`, `discuss-compare.md`, `discuss-explain.md`, `discuss-flaws.md`, each opening with an `**Advisory-only:**` header.
  - Stub surfaces: `.agents/skills/…/SKILL.md` and `.opencode/skills/…/SKILL.md` via `apply-wrapper`; `.opencode/commands/aitask-brainstorm-discuss.md` hand-written.
  - Registration: `settings_app.py` `VALID_PROFILE_SKILLS` + prose copy, `seed/project_config.yaml` comment, `test_brainstorm_discuss_is_a_known_skill`.
  - Goldens: `tests/golden/skills/aitask-brainstorm-discuss/SKILL-{default,fast,remote}-claude.md`, `tests/golden/procs/aitask-brainstorm-discuss/<proc>-default.md` ×4.
  - Tests: `tests/test_skill_render_aitask_brainstorm_discuss.sh` (249 asserts, incl. Test 0c closure-set == the 5 authoring files via `walk_closure(write=False)`), `tests/test_brainstorm_discuss_skill_contract.sh` (7 asserts, two isolated mutants).
- **Deviations from plan:** (1) `apply-wrapper opencode-command` emits the legacy static `@.claude/skills/<skill>/SKILL.md` include, not the profile-aware stub shadow/trail use — the OpenCode command was hand-written to the shadow/trail shape (Test 5 pins it). The Codex / OpenCode skill stubs are generator output and, like trail's, carry no `user-invocable:` line. (2) The contract test adds a second mutant (M2: guardrail drops "node YAML" → exactly `guardrail:node-yaml`) so each check family has a guard-specific red proof, and asserts M1 leaves the guardrail checks untouched.
- **Issues encountered:** render-test needles that span a template line wrap failed (`read and follow \`discuss-explain.md\``) — asserted on the backticked filename instead. `bash tests/run_all_python_tests.sh` → `PYTHON SUITE: FAILED` with 26 failures, all in `tests/test_board_bytrail_view.py` / `test_board_dialog_run_dispatch.py` / `test_board_work_report.py`, while another session has uncommitted `.aitask-scripts/board/aitask_board.py` edits and an untracked `board_trail_screen.py`; the latter two modules pass in isolation (38/38) and by-trail fails 3/147 in isolation — unrelated to this task's files (no board code touched).
- **Key decisions:** the contract test's temp root carries a copy of the shadow skill too, so a shadow reference really resolves (otherwise the closure check could never fire); liveness of M1 is asserted (`aitask-shadow-default-` rendered) before its violations. Step 0 forbids zero-node-id helper calls (that form lists every node). The ambiguous-operand rule asks rather than picks when an operand is a handle of one proposal and the node id of another.
- **Upstream defects identified:** .aitask-scripts/aitask_audit_wrappers.sh:render-wrapper — `apply-wrapper opencode-command` generates the legacy static `@`-include command instead of the profile-aware stub the stub-skill pattern requires, so a new profile-aware skill gets a non-dispatching OpenCode command unless hand-fixed
- **Notes for sibling tasks:** **Final shortcodes (t1823_5 documents them):** `>c` compare in simple words, `>cd` compare in depth, `>e` explain one proposal simply but fully, `>q` free-form Q&A (inline), `>f` structural design-flaw & risk check, `>h` how a proposal evolved via `--lineage` (inline), `>?` reprint menu. Operands: handles (`A`, `B`, … in argv order) or node ids; ambiguous operand → clarification. **Invocation (t1823_3/t1823_4):** `/aitask-brainstorm-discuss <task_num> <node_id> [<node_id>...]` — at least one node id is required; the stub forwards ARGUMENTS unchanged; resolver key for `default_profiles` is `brainstorm-discuss`. The skill runs only the read-only `aitask_brainstorm_context.sh` (already whitelisted by t1823_1) plus file reads — no further helper whitelisting needed.
