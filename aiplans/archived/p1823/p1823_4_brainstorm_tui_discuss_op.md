...
- **Deviations from plan:** `_REPO_ROOT` derives from the existing `AIT_PATH` constant instead of `Path(__file__).parents`, same value, one source. Instead of a separate AST class the new guard coverage extends `_screen_source_files()` with `SHARED_LAUNCH_DIALOGS` (so the existing no-unguarded-bases and no-direct-pop_screen scans cover them) plus a new `SharedLaunchDialogGuardTests` (inspect-based class check + mounted repeated-close tests for AgentCommandScreen, EditStringScreen, AgentModelPickerScreen — the picker mounts headless fine with `load_all_models(REPO_ROOT)`).
- **Issues encountered:** One harness slip in the new launch test (missing-binary case did not set the cursor, so the app's boot-time primary was used) — fixed by setting the primary explicitly. Red proof confirmed before Step 0: all three mounted tests failed with `ScreenStackError`. Negative controls (runtime mutants, no tree edits): moving the cursor check ahead of discuss fails `test_vanished_unmarked_cursor_does_not_block_marked_targets`; rebuilding the wrapper argv on "run" fails both finalized-command "run" tests.
- **Key decisions:** No `_FRESH_WINDOW_OPERATIONS` change — the `agent-` window-name prefix already selects "+ New window" (`should_default_to_new_window`). `_dispatch_argv` is a plain sync method (no `@work`), so no worker can outlive `run_test`. A vanished unmarked cursor with a surviving marked set launches silently (the cursor is not a target); a partly vanished marked set warns naming the dropped ids. Minimonitor companion only on `new_window` (as codebrowser explain).
- **Upstream defects identified:** None
- **Notes for sibling tasks:** t1823_5 (docs): the Operations-dialog row is "Discuss", last in the list, enabled at any selection including the root; reachable only in init/active non-read-only sessions (the dialog's existing gate); launches via the agent dialog (default new tmux window `agent-discuss-<N>` + minimonitor), terminal fallback without tmux. Help (`H`) on the row shows `_OPERATION_HELP["discuss"]`. The shared agent-launch dialogs (`AgentCommandScreen` and children) are now stale-dismiss guarded for every TUI, not just brainstorm. t1823_6 (manual verification): the live checks are listed in this plan's Verification section, including rapid Esc on the Discuss dialog.
