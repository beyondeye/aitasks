...
- **Manual-verification failure:** item "[t717_3] In "Top verified models (recent)" mode, confirm rankings reflect recent-window scores not all-time (a model with high all-time but no recent runs falls below a model with recent activity)." failed; follow-up task t1082.
