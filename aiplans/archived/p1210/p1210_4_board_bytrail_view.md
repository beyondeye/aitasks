...
### Change Request 1 (2026-07-26 00:15)
- **Requested by user:** Two confirmed review findings: (1) BLOCKING — the approved §9.2 state matrix promises a banner note for an archived trail owner, but `_refresh_subtitle()` derived text only from the handle, document, and drift result (`owner_archived` was used solely by `TrailSelectItem`), so selecting an archived-owned trail omitted the note; (2) FOLLOW-UP — the plan specified a `LoadingOverlay` busy affordance for the discovery scan, but `_open_trail_select()` only issued a `notify()` before starting the worker.
- **Changes made:** (1) `_activate_trail` now persists `_trail_owner_id` / `_trail_owner_archived` from the selected `TrailInfo`, and the subtitle composer appends `· owner t<id> archived` in the trail-banner branch; regression test `test_archived_owner_noted_on_banner` covers both the archived and the non-archived (absence) case. (2) The discovery scan now pushes a `LoadingOverlay("Scanning for trails…")`; the callback pops it before the supersession check, and each scan owns its own overlay instance so a stale callback can never pop a newer scan's overlay; `test_z_enters_bytrail_empty_state` additionally asserts no overlay lingers after the scan completes.
- **Files affected:** `.aitask-scripts/board/aitask_board.py`, `tests/test_board_bytrail_view.py` (22 tests, all green).

## Final Implementation Notes
- **Actual work done:** Implemented the full planned surface in `.aitask-scripts/board/aitask_board.py`: `bytrail` base filter (`z` binding, `ViewSelector.BASES` + click target, view-name flow, search placeholder + `a/l/f/i/y/z` hint, `apply_filter` derived-view arm, `refresh_board` early-return branch, `_get_visible_col_ids` union); pure model layer (`TRAIL_CLASSIFICATION_GLYPHS` pinned to the schema enum, `build_trail_lanes`, `trail_entry_refs`, `compute_trail_overlaps`, `dedupe_trail_records` + `_trail_owner_rank`, `load_local_project_name`, `trail_ref_to_local_id`); read-only subprocess helpers (`load_trail_blob`, `_trail_versions`, `run_trail_drift`, `discover_trails` over active metadata + `iter_archived_frontmatter`); widgets (`TrailColumn` with `W<ordinal> · <title>` headers, `TrailTaskCard` with glyph/confidence badges + landed ✔/strike-through, `TrailGhostCard(TaskCard)` with `_GhostTaskStub`, inline fail-closed error/empty Statics with `versions` fallback); modals (`TrailSelectScreen` with owner/scope/freshness/updated + "also in" overlap sub-lines, `TrailDetailScreen` full narrative projection incl. drift reasons); actions (`T` = `action_trail_task` with By-Topic lane-root resolution, By-Trail overloads `s`→selection, `r`→refresh launch, `enter`→detail; `_launch_trail` mirroring `_launch_work_report`); drift-on-entry thread worker + discovery worker with the `_trail_gen` supersession guard and `LoadingOverlay`; `_refresh_board_data`/`action_refresh_board` keyboard-vs-timer split; single `_refresh_subtitle()` composer (incl. §9.2 archived-owner note); `check_action` ghost guard + `bytrail` in all derived-view exclusions + `trail_task` gate. Tests: new `tests/test_board_bytrail_view.py` (22 tests: model, render, Pilot, gating, ghost navigation, focused-ghost footer regression, auto-refresh regression, supersession negative control, subtitle restore, fold dedup, launch construction spies, read-only negative controls, error-state render, archived-owner banner); `tests/test_board_work_report.py` derived-views loop extended with `bytrail`.
- **Deviations from plan:** (1) The §9.2 error card is an inline `Static` with class `trail-error` instead of a dedicated `TrailErrorCard` class (tests select by class). (2) `TrailTaskCard`/`TrailGhostCard` additionally carry `trail_wave` so the detail modal gets wave context without a DOM walk. (3) The read-only negative control runs at the helper level (`load_trail_blob` + `run_trail_drift` under a `subprocess.run` spy with a get/versions/drift verb allowlist) rather than a full Pilot entry cycle — thread workers make the full-cycle variant racy; the helper spy plus the board-level "only these helpers are called" seam gives the same guarantee. (4) Discovery initially used `notify()` instead of the planned `LoadingOverlay` — restored to the overlay in review round 1 (see Post-Review Changes).
- **Issues encountered:** The inherited `TaskCard.on_click` double-click path calls `action_toggle_children` directly, bypassing its `check_action` gate — `TrailTaskCard`/`TrailGhostCard` override `on_click` to route double-click to `action_view_details`. The full-battery exit code initially ran through a `| tail` pipe (exit code masked); re-run unpiped: 2094 tests OK. PyPy fast-path compile + import verified (`(_common_types_metatype, …)` startup noise is pre-existing pyyaml-under-PyPy output, reproduced with a bare `import yaml`).
- **Key decisions:**
  - Keys (user-confirmed at planning): `z` = view switch (base family), `T` = contextual trail action; in-view `s`/`r`/`enter` overloads follow the In-Flight overload precedent and the RFC §15 footer.
  - Ghost cards subclass `TaskCard` so every focus/nav/refocus seam works unchanged; safety is the explicit `check_action` ghost guard (primary) plus a defensively complete `_GhostTaskStub` (backstop) — `manager.is_modified` reads `.filepath`, so a near-miss stub would crash footer evaluation.
  - Auto-refresh timer routes to `_refresh_board_data()`; only the `r` keypress reaches the By-Trail refresh-launch branch (mutation-checked: reverting the tick to `action_refresh_board()` turns `test_auto_refresh_tick_never_launches_in_bytrail` red).
  - `_trail_gen` bumped on every re-entry point (enter/leave view, activation, selection reopen); worker callbacks re-check `(gen, base_filter, handle)` before mutating state.
  - Discovery dedupes by handle with pinned owner precedence (active non-folded > active folded > archived, tie → lowest id) because `_fold_transfer_artifacts` copies without stripping the folded file's entry.
  - The banner rides `Header.sub_title` through the single `_refresh_subtitle()` composer, which restores the auto-refresh text on view exit.
- **Upstream defects identified:** None
- **Notes for sibling tasks:**
  - T5 (t1210_5, move commands): seams in place — `TrailColumn.wave` (wave dict), per-card `trail_entry` / `trail_view` / `trail_wave`, and `is_ghost=True` on ghost cards (exclude from moves; the `check_action` ghost guard already hides `move_task_*` when a ghost is focused, and all `move_*` actions return False in `bytrail` — T5 must carve its `m`/`M` gates INSIDE the bytrail branch, not by removing the exclusions). `m`/`M` keys verified free at board scope.
  - T6 (t1210_6, docs): document keys `z` (view) / `T` (create-refresh action) / in-view `s`/`r`/`enter`; boardcol/boardidx are never digested, so moves don't stale a trail; the banner formats as `By-Trail: "<title>" (⚠ stale: <n>)[ · owner t<id> archived]`.
  - The live repo still has no `artifacts:`-carrying task; t1210_7 (manual verification) should create a real trail via `/aitask-trail` and walk the view against it (discovery, selection, badges, detail, `r` refresh launch, error card via a deliberately broken handle).
