...
Archived Sibling Plans: aiplans/archived/p163/p163_1_vocabulary_files_and_install.md, aiplans/archived/p163/p163_2_add_reviewguide_metadata.md, aiplans/archived/p163/p163_3_reviewguide_scan_script.md, aiplans/archived/p163/p163_4_classify_skill.md
...
Child task t163_5 of the review guides consolidation (t163). Creates a new Claude Code skill `.claude/skills/aitask-reviewguide-merge/SKILL.md` for comparing two similar reviewguide files and merging, deduplicating, or keeping them separate. The classify skill (t163_4) populates `similar_to` fields; this skill acts on those to consolidate overlapping review guides.
...
```bash
mkdir -p .claude/skills/aitask-reviewguide-merge
```

Create `.claude/skills/aitask-reviewguide-merge/SKILL.md` — single new file, the only deliverable.
...
| File | Action |
|------|--------|
| `.claude/skills/aitask-reviewguide-merge/SKILL.md` | **new** (main deliverable) |
...
5. Verify vocabulary file update paths use `aireviewguides/*.txt`

## Final Implementation Notes

- **Actual work done:** Created `.claude/skills/aitask-reviewguide-merge/SKILL.md` (280 lines) with single-pair mode (Steps 1-7: fzf resolution, semantic bullet comparison, merge/keep-separate execution) and batch mode (Steps 8-13: --find-similar scanning, environment filtering, pagination, looping). Also created two test reviewguide files for manual testing.
...
- **Notes for sibling tasks:** This is the last child task of t163. The merge skill does NOT modify seed (same as classify). Vocabulary file paths are `aireviewguides/*.txt`. Test files (`test_security_expanded.md`, `test_code_quality.md`) should be deleted after testing.
