...
- **Actual work done:** Implemented exactly as planned. (1) `live_check.py`: added `_DOCS_URL` / `_INVITE_URL_TMPL` constants and `_invite_hint()` builder; `_FIX_VISIBILITY`/`_FIX_PERMISSIONS` now reference `https://www.aitasks.io/docs/workflows/bug-report-intake/`; bot id captured post-connect via new `DiscordAdapter.self_id` read-only property and spliced (digits-only guard) into all five visibility/permission failure sites. (2) Swapped `aidocs/chat/chatlink_sandbox.md` for the public URL in `preflight.py` (docker fix_hint), `daemon.py` (stderr warning), `lib/sandbox_launch.py` (LaunchError). (3) Tests: `FakeAdapter` gained constructor-overridable `self_id="1234567890"`; six new checks (invite URL in visibility + permission hints, docs-page reference, `self_id=None` fallback, non-digit hygiene negative control). (4) Docs: server-invite vs. per-channel permission-overwrite paragraph + "bot lacks access to the intake channel" troubleshooting row on the website page; matching clarification after step 6 in `aidocs/chat/discord_bot_setup.md`. (5) Coordination: t1149_4 pointer updated to "t1189 landed first — carry content forward"; t1184 checklist rows 29–30 now expect docs link + invite URL in the fix hints.
- **Deviations from plan:** None material. One extra touch: the `daemon.py` comment "(Legacy stderr text preserved verbatim…)" was reworded — no longer verbatim after the URL swap. The `REQUIRED_BOT_PERMISSIONS` pairing comment kept pointing at `aidocs/chat/discord_bot_setup.md` step 5 (still the canonical enumerated permission list; code comments citing aidocs are explicitly allowed by the task).
- **Issues encountered:** None. All suites pass: `test_chatlink_wizard.sh` 54, `test_chatlink_tui.sh` 66, `test_chat_discord.sh` 167; `py_compile` clean; `hugo build --gc --minify` clean; final grep sweep confirms no user-visible `aidocs/` strings remain under `.aitask-scripts/chatlink/` + `.aitask-scripts/lib/`.
- **Key decisions:** Public `self_id` property on `DiscordAdapter` (Discord-specific, like `fetch_bot_permissions`) instead of live_check reaching into `_self_id`; `bot_id.isdigit()` guard preserves the fixed-template token-hygiene contract (pinned by a negative-control test with a token-bearing non-digit id); invite URL appended to ALL five `_FIX_VISIBILITY`/`_FIX_PERMISSIONS` sites (including timeouts) rather than only the two named failure rows — same remedy, simpler code.
