...
- **Actual work done:** Rewrote pseudo-verb lines in all 6 brainstorm templates under `.aitask-scripts/brainstorm/templates/` exactly as designed. Each `report_alive: "<msg>"` / `update_progress: <N>` / `check_commands` / `update_status: <status>` line was replaced with an explicit reference to the corresponding H2-section procedure in `<agent>_instructions.md` ("Heartbeat / Alive Signal", "Progress Reporting", "Reading Commands", "Status Updates"). Block counts: initializer/patcher/synthesizer = 4 Checkpoints + Completion (15 reference lines each); explorer/comparator/detailer = 3 Checkpoints + Completion (12 reference lines each). Total: 81 inserted, 81 deleted across 6 files.
- **Pre-implementation verification:** Confirmed the four H2 section names referenced in the plan (`## Status Updates`, `## Progress Reporting`, `## Heartbeat / Alive Signal`, `## Reading Commands`) match exactly what `aitask_crew_addwork.sh` lines 207–252 emits into `<agent>_instructions.md`. No drift from the plan.
- **Deviations from plan:** None. The line numbers in the plan's "Files to edit" table were approximate; actual offsets shifted by ±1 line due to surrounding content drift, but the grep-based identification pattern was exact and made line numbers immaterial.
- **Issues encountered:** None.
- **Key decisions:** None — followed the rewrite pattern verbatim from the plan.
- **Verification result (static, Step 1):** `grep -nE "report_alive|update_progress|check_commands|update_status" .aitask-scripts/brainstorm/templates/*.md` returns zero matches across all 7 template files (including `_section_format.md`). `grep -c "Execute the \*\*"` confirms 12 / 15 / 15 / 12 / 15 / 12 reference lines in detailer / initializer / synthesizer / comparator / patcher / explorer respectively — exact match to expected counts.
- **Verification deferred (Steps 2 and 3):** Manual end-to-end test (`ait brainstorm bootstrap`) and DEAD-check are user-driven — they require an actual brainstorm run with a small imported proposal. If those fail, follow the plan's Step 4 fallback: close this child Done with a "fix did not stick" note here, then pick up `/aitask-pick 650_3`.
- **Notes for sibling tasks:** If verification steps 2–3 succeed, t650_3 should be archived without code changes per this plan's "Notes for sibling tasks" guidance — the procedure-reference approach is the lowest-blast-radius fix and avoids a substitution engine entirely. The pattern established here ("Execute the **\<H2 section\>** procedure from your `_instructions.md` with \<param\>: \<value\>") is the canonical phrasing for any future template that needs to invoke a documented procedure from `<agent>_instructions.md` — mirrors the Manual Verification reference style used in `task-workflow/SKILL.md`.
