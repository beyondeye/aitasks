...
- **Actual work done:** Created the seeder helper script `.aitask-scripts/aitask_create_manual_verification.sh` (wraps `aitask_create.sh --batch --type manual_verification` plus `aitask_verification_parse.sh seed`, outputs `MANUAL_VERIFICATION_CREATED:<id>:<path>`). Inserted the aggregate-sibling sub-procedure in `.claude/skills/task-workflow/planning.md` between the child-plan-commit block (line 169) and the child task checkpoint. Added a new "Manual Verification Follow-up (single-task path)" sub-procedure at the end of the Checkpoint section, with explicit dispatch from both the profile `"start_implementation"` short-circuit and the interactive "Start implementation" option. Added the Explore-path follow-up prompt at the end of `.claude/skills/aitask-explore/SKILL.md` §3 Task Creation. Updated all 5 whitelist files (runtime + seed for Claude, Gemini, OpenCode) with entries adjacent to `aitask_create.sh`.
- **Deviations from plan:** Edit 2 placement refined. The original plan said "at the end of §6.1 after ExitPlanMode"; implementation placed it inside the Checkpoint section's "Start implementation" branch (both the profile-driven and interactive paths). This keeps the follow-up prompt from firing on "Revise plan" / "Approve and stop here" / "Abort", which is the correct semantic. Also added three skip guards to the single-task path: skip when `is_child == true` (aggregates cover children), skip when current task already has `issue_type: manual_verification`, and skip when child tasks were just created (aggregate handled it). In the aggregate-sibling sub-procedure, added a skip-if-one-child guard (aggregates need ≥2 children to be useful). The seeder script also commits the post-seed checklist edit with `./ait git` (the `aitask_create.sh --commit` step only covers the initial frontmatter+body, and the `seed` step modifies the file afterwards — this was not mentioned in the plan but is necessary for the commit to be atomic).
- **Issues encountered:** Shellcheck flagged `--labels verification,manual` because the unquoted comma looks like an array separator; fixed by quoting the value. Shellcheck also flagged the backtick-delimited markdown code span in the description preamble (SC2016); suppressed with a targeted disable comment since the backticks are intentional markdown.
- **Key decisions:** The seeder accepts `--parent <N>` XOR `--related <id>`, mirroring the two callers' needs. `--related` falls back to `--deps` in `aitask_create.sh` (since there is no `--related` flag there) and appends `**Related to:** t<id>` in the body. Checklist bullets in the aggregate flow are prefixed with `[t<parent>_<child>]` so at verification time the user can see which child each item belongs to without cross-referencing plans.
- **Notes for sibling tasks:**
  - **t583_8 (documentation):** should document both prompts (aggregate-sibling and single-task follow-up) and the Explore-path variant in `website/content/docs/workflows/manual-verification.md`. The three skip guards (is_child, issue_type, children just created) should be called out so users understand why the prompt doesn't always appear.
  - **t583_9 (meta dogfood / aggregate verification):** this is the first sibling that could consume the aggregate-sibling flow end-to-end. Run `/aitask-pick 583` (when all other siblings are done) to validate the aggregate path against real children.
  - **Gemini / Codex / OpenCode skill mirrors:** per CLAUDE.md the Claude Code versions of `planning.md` and `aitask-explore/SKILL.md` are the source of truth. Follow-up tasks should port the two sub-procedures (aggregate-sibling + single-task follow-up) into the corresponding agent trees once this task archives.
  - **Insertion-point drift risk:** `planning.md` grows when the framework evolves; the aggregate-sibling sub-procedure is keyed to "after the child plan commit, before the child checkpoint" rather than to an absolute line number. If someone later inserts new steps there, verify the order is preserved.
