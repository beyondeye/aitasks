...
- **Issues encountered:** None — shellcheck clean, all 60 tests pass
- **Key decisions:** Skip posting when existing comment found (rather than updating) — keeps the implementation simple. The workflow condition fix prevents re-triggers on GitHub; the idempotency check prevents duplicates on scheduled GitLab/Bitbucket runs.
