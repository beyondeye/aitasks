...
### Change Request 6 (2026-08-05 09:40)
...
- **Actual work done:** Extended the risk-mitigation machinery with inline dispositions exactly as planned: four-timing line contract with `inline_risk`/`added_complexity` metrics and recommended dispositions; per-mitigation propose-and-confirm flow (prose-first details, compact widget recap, NON-SKIPPABLE banners on the disposition and recovery prompts); inline phases as name-labeled `### Pre-phase/Post-phase (risk mitigations)` blocks with deterministic fallback anchors; post-inline single-pass risk reassessment with durable-provenance bullet rules; Parts 2/3 rebuilt as a reconcile-converge engine (three work partitions, exact-identity adoption probe with comma-delimited provenance prefix, per-item committed `created: t<id>` witnesses, ID-retaining `created: dropped(t<old>)` terminal marker, idempotent append/subtract convergence of `depends:`/`risk_mitigation_tasks`/links, `risk_before_blocking` stop signal keyed on unfinished-not-created); supporting edits in risk-evaluation.md, planning.md §6.1, the plan contract, SKILL.md Step 7/8d, and the three website pages; goldens + Test 7 (header cap) added; all rendered variants refreshed.
- **Deviations from plan:** Eight review rounds hardened the original design well beyond the plan: `risk_before_created` was renamed to `risk_before_blocking` with unfinished-based semantics; the Part 2/3 idempotency guard originally left to t1331 was implemented here (t1331 narrowed accordingly, with its task file annotated); the question-payload requirement from round 1 was partially rolled back in round 6 after live use showed an unreadable blob — prose-first with a minimal widget recap is the final form.
- **Issues encountered:** The rerender driver skips `task-workflow-*` dirs as direct targets (no `.j2`), but entry-point closure walks refresh them — verified by content greps. A concurrent session's t1293 monitor/parser changes shared the worktree; the commit stages the t1419 set explicitly.
- **Key decisions:** Name-based cross-reference identity over positional; witness as both creation record and reconciliation input; ID-retaining dropped marker to make plan-commit-first ordering crash-safe; per-plan mitigation-name uniqueness as a contract requirement; the >4-mitigation live prompt-batching behavior is accepted as unverified (user declined a live-walk follow-up task).
