...
- **Actual work done:** Created `.claude/skills/aitask-wrap/SKILL.md.j2` authoring template (frontmatter `name: aitask-wrap-{{ profile.name }}`, Step 1b wrapped in `{% if agent == "claude" %}`, all `../task-workflow/...` refs rewritten to full-path form so the walker auto-rewrites them per agent root). Replaced the unified body in the four agent stubs (`.claude/skills/aitask-wrap/SKILL.md`, `.agents/skills/aitask-wrap/SKILL.md`, `.gemini/commands/aitask-wrap.toml`, `.opencode/commands/aitask-wrap.md`) with the canonical 21-line dispatch stub from `aidocs/stub-skill-pattern.md §3a-§3d`. Generated 6 goldens under `tests/golden/skills/aitask-wrap/` (3 profiles × {claude, codex}). Wrote `tests/test_skill_render_aitask_wrap.sh` (124 assertions covering golden diffs, agent gate behavior, Jinja-marker absence, forbidden runtime tokens, walk-write ref rewrites, and stub markers). Created `aidocs/agent_runtime_guards_audit.md` cataloging the two remaining runtime guards in `task-workflow/{SKILL.md,planning.md}` and explaining why their conversion is deferred (Test 1b cascade across 9 caller skills).
- **Deviations from plan:** (a) Plan called for deleting `.opencode/skills/aitask-wrap/SKILL.md` and the directory. During implementation I discovered the precedent skill `aitask-fold` keeps BOTH `.opencode/skills/aitask-fold/SKILL.md` AND `.opencode/commands/aitask-fold.md` (parallel stubs with identical bodies). To match that precedent I overwrote `.opencode/skills/aitask-wrap/SKILL.md` with the canonical stub instead of deleting it. (b) Plan inventory called out 3 occurrences of "If running in Claude Code"/`~/.claude/plans` in shared skill files; on closer inspection one of those (`plan-externalization.md`) is Claude-only file content, not a guard — the audit document distinguishes A (callsite guards in `task-workflow/{SKILL.md, planning.md}`) from B (Claude-only procedure body) and C (the now-Jinja-gated occurrence in `aitask-wrap`).
- **Issues encountered:** None blocking. Confirmed via `md5sum` comparison that codex/gemini/opencode renders are byte-identical to each other within each profile, supporting the choice of codex as the canonical "non-claude" golden agent (6 goldens instead of 12).
- **Key decisions:**
  - Goldens dimensionality: `claude × {default, fast, remote}` + `codex × {default, fast, remote}` = 6 goldens. codex stands in for "non-claude" per the byte-equivalence proven by Test 1b's agent-invariance subassertions (gemini and opencode renders must equal codex).
  - Test 1b purpose flipped: instead of asserting agent invariance (the pattern in `test_skill_render_aitask_fold.sh:91-105`), aitask-wrap's Test 1b asserts agent DIVERGENCE for the gated content — claude render contains "Step 1b: Check for Recent Claude Plans" and `~/.claude/plans`, all other agent renders do not.
  - Resolver short name `wrap`, matching the `aitask-` prefix-stripping convention used by `fold`, `pick`, `explore`, `review`, `qa`, etc.
  - Reference paths rewritten from `../task-workflow/...` to full-path `.claude/skills/task-workflow/...` form so the walker auto-rewrites them per agent root (verified by Test 4 walk-write assertions).
- **Upstream defects identified:** None. All adjacent test suites (`test_skill_render_aitask_fold.sh`, `test_skill_render_aitask_pick.sh`, `test_skill_render_task_workflow.sh`, `test_skill_render_uniform.sh`, `test_skill_rerender.sh`, `test_skill_verify.sh`, `test_opencode_skill_legacy_pointers.sh`, `test_skill_parity_runtime_vs_rendered.sh`) pass without regressions.
