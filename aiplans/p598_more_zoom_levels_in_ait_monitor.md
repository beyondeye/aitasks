...
- **Actual work done:** Replaced the single `"fullscreen"` XL preset in `.aitask-scripts/monitor/monitor_app.py` with three `"agents:N"` dynamic presets (XL_9 / XL_6 / XL_3). `_apply_preview_size()` parses `agents:N` and sets `section_h = max(PREVIEW_MIN_SECTION_H, screen_h - reserve)` where `reserve = PREVIEW_LAYOUT_FIXED_LINES + N * PREVIEW_AGENT_CARD_LINES`. `on_resize()` now re-applies on any `agents:N` spec. Removed the old `PREVIEW_FULLSCREEN_RESERVE` constant. Updated `website/content/docs/tuis/monitor/reference.md` (preview-size table → 6 rows) and `how-to.md` (six-preset copy).
- **Deviations from plan:** During manual verification the user reported an off-by-one: XL_3 fit only 2 agents, XL_6 fit 5, XL_9 fit 8. Root cause: `#pane-list` has `border: solid` in the CSS (`monitor_app.py` lines 372-379), which consumes 2 lines of the outer `1fr` height that the sizing formula did not account for. Fix: bumped `PREVIEW_LAYOUT_FIXED_LINES` from `3` (header + session-bar + footer) to `5` (those three + 2 lines of `#pane-list` border). Labels now match the displayed agent count exactly on terminals ≥ header+footer+2N+5 lines tall.
- **Issues encountered:** On workflow entry, the task file `aitasks/t598_more_zoom_levels_in_ait_monitor.md` was missing from the `aitask-data` branch even though `aitask_pick_own.sh` had returned `OWNED:598` (and printed `Updated: aitasks/t598_…md`). Investigation via `git reflog` in `.aitask-data/` found that a concurrent session's `pull --rebase --quiet` sequence had discarded the Add/Start commits (dangling at `334203b4` and `31703525`). Recovered by `git cherry-pick 334203b4 31703525` in `.aitask-data/`, then `./ait git push`. Worth watching: if it recurs, `aitask_pick_own.sh --sync` or the lock commit machinery may need a safeguard when multiple Claude sessions operate on the same `aitask-data` branch concurrently.
- **Key decisions:**
  - Encoded the dynamic agent-count spec as a string sentinel `"agents:N"` in the existing `PREVIEW_SIZES` tuple rather than widening the tuple shape or introducing a dataclass. Keeps one table, one dispatch point, minimal code churn; call-sites that only need the label (`_apply_preview_size` notify) are unaffected.
  - Kept `S / M / L` as fixed numeric presets rather than converting them to `agents:N` as well — they have a different intent (preview-height-first) and users already have muscle memory for those modes.
  - Bumped `PREVIEW_MIN_SECTION_H` to 4 and `PREVIEW_MIN_PREVIEW_H` to 2 so XL_9 on very short terminals still renders *some* preview rather than collapsing the section entirely.
- **Build verification:** `python3 -m py_compile .aitask-scripts/monitor/monitor_app.py` passes. No project-level `verify_build` configured in `aitasks/metadata/project_config.yaml`, so Step 9 will skip automated build verification.
