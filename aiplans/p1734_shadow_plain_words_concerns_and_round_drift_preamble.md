...
- **Actual work done:** Everything in the plan landed: the shared procedure `.claude/skills/aitask-shadow/round-preamble.md` (audience rule, plain-words line, the six "Where this is heading" headings with the fixed verdict rule, framework-vocabulary mapping, per-producer subject table, snapshot protocol); the two-placement rules in all four producers plus the snapshot step (plan producers snapshot the plan, impl-challenge snapshots the composite diff); the `snapshot` / `snapshots` verbs in `aitask_shadow_rejected.sh` with `--kind plan|diff` and `--partial`; `tests/test_shadow_snapshot.sh` (49 assertions); four new guard classes in `tests/test_concern_parser.py` (`TestProducerPlainWordsRule`, `TestProducerRoundPreambleRule`, `TestPlanProducerSnapshotStep`, `TestRoundPreambleContract`) with negative controls and rendered-surface twins, including both inline post-phase mitigations; `round-preamble` added to the render test's invariant list; the three `impl-challenge` proc goldens regenerated (additions only); the framework doc and the website page.
- **Deviations from plan:** None of substance. The rejection-store doc section got a short cross-reference paragraph instead of a full "Plan snapshots" subsection, because the snapshot description lives once under the review-loop section beside the preamble it serves. `_fence_bodies` was lifted to module level and `_example_item_lines` now delegates to it (the nested walker in the round-header test class was left in place).
- **Issues encountered:** (1) `bash tests/run_all_python_tests.sh <path>` runs the whole suite serially rather than the one module — it was stopped after several minutes and the module run directly with the venv's pytest (184 passed). (2) Shellcheck flagged a `found` name clash (an array elsewhere in the file) and a variable modified inside the `snapshots` sort subshell; both renamed. (3) Three shadow-review concerns were folded into the plan before approval: implementation rounds have no git baseline for "since last round" (round-keyed diff snapshot, heading 2 made load-bearing), a capture-window snapshot can silently mix plan generations (three-way source preference, latest-rendering delimiting, `--partial`), and the producer guards did not pin the shared procedure's contract (`TestRoundPreambleContract`). A fourth, post-implementation concern (pipefail/SIGPIPE in `assert_contains`) did not reproduce: the render suite sets only `set -e`, haystacks are ~34 KB (under the pipe buffer), five runs exit 0 at 505/505.
- **Key decisions:** Prose-only additions, both before the block — the parser, picker and `compose_recheck_prompt` are untouched, pinned by the `concern-format.md` note and the example-fence guard. The snapshot verbs reuse the rejection store's directory, id validation, mutex and atomic write rather than a parallel store, so `prune` covers them for free. `SNAPSHOT:<kind>|<round>|<state>|<path>` puts the path last (split on at most three `|`). `plan-diagnose-errors.md` compares its own earlier findings and takes no snapshot (its subject is the error picture, not a document).
