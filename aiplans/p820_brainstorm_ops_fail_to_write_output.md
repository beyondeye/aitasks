...
- **Actual work done:** Added a 3-line note to the `## Writing Output` section
  of the `_instructions.md` body generated by `.aitask-scripts/aitask_crew_addwork.sh`,
  telling the agent the `<agent>_output.md` placeholder pre-exists and to read
  it once before writing. Added a self-contained regression test
  `tests/test_crew_addwork_output_instructions.sh` (6 assertions).
- **Deviations from plan:** None. Implemented exactly as planned (the
  user-approved "instruct the agent" approach over the "remove the placeholder"
  alternative).
- **Issues encountered:** None. The edit is to a string literal; shellcheck
  reports only the pre-existing SC1091 (sourced-lib info) and SC2001 (style nit
  on line 173) — neither introduced by this change.
- **Key decisions:** Kept the note in the generated `_instructions.md` rather
  than the brainstorm `templates/*.md`, because `_instructions.md` is the
  canonical agent-agnostic location every crew agent consults for the output
  path, and the fix then applies to every agent type (explorer, comparator,
  synthesizer, detailer, patcher, initializer) and every crew use case, not
  just brainstorm. The placeholder pre-creation itself was deliberately left
  intact (it is part of the crew file contract and is committed at addwork
  time).
- **Upstream defects identified:** None.

### Verification results
- `bash tests/test_crew_addwork_output_instructions.sh` → 6 passed, 0 failed.
- `bash tests/test_crew_groups.sh` → 24 passed, 0 failed.
- `bash tests/test_crew_status.sh` → 57 passed, 0 failed.
