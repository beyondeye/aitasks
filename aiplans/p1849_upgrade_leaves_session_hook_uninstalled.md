...
### Change Request 1 (2026-09-25 12:05)
- **Requested by user:** four verified defects — (1) the runtime probe only located `python3`, so a broken `python3` with a working `AIT_PYTHON` read as OK; (2) a failed write of `.claude/settings.json` still printed "Merged" and returned 0; (3) `--hooks-only --yes` still prompted under a terminal; (4) the NO_SEED advice sent installed projects to `ait setup`, which cannot restore a seed there (installed releases delete `seed/`).
- **Changes made:** (1) `claude_session_hook_runtime` now executes `python3 -c 'import json, sys'` and reports `PYTHON3_BROKEN:<path>`; the repair names the framework wrapper case (full `ait setup` rebuilds it) separately from a foreign `python3` (repair/replace it). (2) `merge_claude_hooks` guards the write and returns 1 with "Could not write"; `setup_claude_hooks` guards `mkdir`/`cp` too. (3) `setup_claude_hooks --yes` skips the prompt in every mode; `--hooks-only --yes` passes it through. (4) New `claude_session_hook_seed_repair`: `ait setup` only when `seed/claude_settings.hooks.json` exists (source checkout), otherwise the installer (`ait upgrade` for a newer release, or the documented `install.sh --force` reinstall). Docs wording updated for `--yes` and a non-running `python3`.
- **Files affected:** `.aitask-scripts/lib/claude_hook_status.sh`, `.aitask-scripts/aitask_setup.sh`, `tests/test_claude_hook_status.sh`, `tests/test_setup_hooks_only.sh`, `website/content/docs/commands/setup-install.md`. Each fix has a test that a targeted mutant fails.

### Change Request 2 (2026-09-25 12:20)
- **Requested by user:** (1) the guarded write still redirected onto `.claude/settings.json`, so a failed write (file-size limit) left it truncated to invalid JSON — use `lib/atomic_write.sh` per the shell conventions; (2) the installed-layout NO_SEED reinstall command lacked `--dir`, so copying it from a project subdirectory targets the wrong directory.
- **Changes made:** (1) `merge_claude_hooks` writes through `ait_atomic_write_text` (dot-prefixed temp beside the file, then rename); because a rename would replace a read-only file that the old redirect refused, it now checks `-w` first and refuses a non-writable `settings.json` explicitly. (2) `claude_session_hook_seed_repair` appends `--dir <resolved project root>` (`printf %q`-quoted) to the reinstall command.
- **Files affected:** `.aitask-scripts/aitask_setup.sh`, `.aitask-scripts/lib/claude_hook_status.sh`, `tests/test_claude_hook_status.sh` (RLIMIT_FSIZE case: original byte-for-byte intact, no temp left), `tests/test_setup_hooks_only.sh` (`--dir` assertion; read-only message). Both fixes' mutants fail their tests.

## Final Implementation Notes
- **Actual work done:** New `lib/claude_hooks_merge.py` (merge/check — the single "is the hook installed" identity, moved out of the `merge_claude_hooks` heredoc) and `lib/claude_hook_status.sh` (status, runtime-with-reason, per-reason repair advice, hints, seed-repair advice). `aitask_setup.sh`: `SETUP_LIB_DIR`, `setup_claude_hooks` 0/1/2 contract with `--yes`, already-installed short-circuit and INVALID refusal, new `setup_hooks_only`, `--hooks-only`/`--yes` flags, `setup_claude_hooks || true` in the full setup, atomic + writability-guarded merge write. `install.sh`: `EXISTING_INSTALL` + `report_claude_session_hook` after the commits. `ide_frozen_offer.sh`: forward-looking note next to the gone-viewer list. Docs: "Installing only the hook" in `setup-install.md`.
- **Deviations from plan:** The `ait ide` tests live in the new `tests/test_claude_hook_status.sh` (Group F) rather than in `tests/test_ide_frozen_offer.sh` — the existing file drives a fixed `/proj` root, and the note needs real fixture projects; the existing file still passes unchanged (its `/proj` has no seed → silent). Two review rounds added: a `PYTHON3_BROKEN` runtime reason, guarded + atomic writes, `--yes` honoured under a terminal, layout-aware NO_SEED advice with `--dir` (see Post-Review Changes).
- **Issues encountered:** Tests re-point `SCRIPT_DIR` at fixture projects after sourcing setup, so helpers are resolved through a source-time `SETUP_LIB_DIR`. Under a temp `HOME`, `~/.aitask/bin/python3` (the framework wrapper) exits 127, so the new tests strip `~/.aitask/*` from `PATH`. `test_frozen_agents_acceptance.sh` refuses to run inside tmux and was not run.
- **Key decisions:** Status before consent before runtime-gated writes in `--hooks-only`; the status lib is sourced lazily (indented) so it never joins a startup closure; resolve_python's memo is bypassed so the probe mirrors a fresh writer process; the ide note is explicitly not causal because `GONE:` lines carry no agent kind; FreezeConfirmDialog note deferred (concurrent uncommitted edits in `monitor/`).
