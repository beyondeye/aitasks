...
- The `category` column costs a second `resolve_category` pass over the archived tree — measured +62 ms on a 248 ms baseline (+25%), paid on every `ait stats` and every stats-TUI load, for a column that only materializes with `--csv`. · severity: medium · → mitigation: t1585
...
- ~150 lines of render logic (three level axes, per-axis scratch clamp counters, two different row-membership rules) land in `aitask_stats.py`, and t1544_5 will want a subset of the same ordering/subtotal logic for its panes. · severity: low · → mitigation: t1586
