...
| `create_worktree` | bool | no | `true` = worktree mode; `false` = current branch. The fork itself is cut at Step 7, after plan approval and the remote drift check | Step 5 (resolve), Step 7 (fork) |
