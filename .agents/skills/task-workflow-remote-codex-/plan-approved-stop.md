...
- **Ensure the plan file is committed** (idempotent — may be a no-op if the Plan
  ...
  contract** in `.agents/skills/ait-git-remote-codex-/SKILL.md`. Exit 2 / `NOCHANGE` **is** the
...
- **Commit the status revert and push:**
  ...
  `.agents/skills/ait-git-remote-codex-/SKILL.md`. Name the task's own file — the old
