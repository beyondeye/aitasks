...
- Display "Profile: using existing plan". Copy plan to `.aitask-data-updated/plan_t<task_id>.md` if not already there. Skip to **Step 5 Checkpoint**.
...
Display: "Profile: proceeding to implementation". Proceed to Step 6.
...
- `create_worktree`, `base_branch`, `output_branch`, `enableFeedbackQuestions` — always works on current branch and never asks feedback questions
