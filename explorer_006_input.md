...
## Exploration Mandate
I want to prepare this proposal for actual implementation. I need I final pass removing references to external materials (like previous proposals). material from external references that are important to understanding the proposal should be simply integrated directly. clean up the proposal from what is not needed from implementation. reorganize the material in the proposal and check repetions and contradictions, so that it can be more easily split in modules for implementation with suggestions of which part of which modules come first. this is not yet planning for implementaiton. it is organizing the proposal in a way that make it easier to map it into actual aitasks. each aitask will reference the full proposal but also will address a specific module in the proposal. but module implementation planning is not in scope for this exploration. this exploration is for final organization and clean up of the proposal. at implementation time, each module will be mapped to a parent task that will be split in child tasks. it would be helpful for each module in the proposal that will be mapped to a task, if it can also split in submodules so that we can establish dependencies and implementation order between submodules of different modules.

## Baseline Node
- Metadata: .aitask-crews/crew-brainstorm-1812/br_nodes/n013_synthesizer_004.yaml
- Proposal: .aitask-crews/crew-brainstorm-1812/br_proposals/n013_synthesizer_004.md
...
- assumption_cadence_bounds_exposure
...
- assumption_wrong_positive_claim_only_overselects
...
- tradeoff_auto_policy_exposure_window
