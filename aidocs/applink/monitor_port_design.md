...
content_transport.md assigns delta computation to the server (row hashing + changed-row collection, Stage 2). It runs in **`monitor_core`** as a helper invoked by the applink listener — not in `monitor_app.py`'s render loop — so the Textual UI and the applink listener share one capture pipeline and the row-hash cache exists once per pane regardless of how many clients are attached. The existing per-pane change tracking (`_last_content` / `_last_change_time`, `tmux_monitor.py:459-465`) stays separate: it feeds idle detection at whole-pane granularity, while the deltifier hashes per-row; merging them is a non-goal.

### Append fast-path detection

The Stage 3 `append` fast path requires bottom-cursor + no-upper-changes detection ([content_transport.md §append](content_transport.md#append)). This check lives next to the deltifier in `monitor_core`: it already has the previous and current row sets in hand, so the bottom-growth test is a cheap prefix comparison before falling back to a full delta.
