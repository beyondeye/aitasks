...
| `NextSiblingDialog` / `ChooseSiblingModal` | `monitor_shared.py` | Two-step: `pick_next_sibling` with no `sibling_id` → `res` `{suggested:{id,title,followup_kind}, current:{id,title,status}, parent_id, ready_siblings:[{id,title,blocked_by,followup_kind},…]}` → mobile either re-sends with the chosen `sibling_id` to execute, or drops it. `blocked_by` lists same-parent sibling ids that are not yet `Done` (the row is still pickable); `followup_kind` is `null` unless the task is an auto-spawned follow-up (vocabulary: `lib/followup_kinds.py`). The desktop suggest-then-choose flow (`action_pick_next_sibling` in `monitor_app.py`) collapses into one round-trip plus the confirmed call. |
