...
**Cross-network fallback (Phase 2, available):** When the PC and phone are on different networks, the user reaches the PC through a mesh VPN or user-run tunnel and advertises that endpoint in the pairing QR via the advertised-endpoint override — recipes in [tunnel_howto.md](tunnel_howto.md). A first-class relay-server transport may be added later — see "Versioning" for how to introduce it without breaking v1 clients.
...
- **Subscription:** mobile sends a control-plane `subscribe` verb with the panes to follow, focused/idle cadences, and a forced-keyframe interval. An **empty or absent `panes` list means "all currently-discovered panes"** (the roster the server enumerates at subscribe time) — so a client can subscribe to everything without first learning pane ids. The `panes` set is the **status (roster) set** — every pane gets `pane_status` badges; an optional `content_panes` field selects the narrower **content set** that gets heavy binary frames (absent ⇒ all panes stream, the legacy default). `focus` raises one pane's cadence and selects the content pane; `request_keyframe` (valid for content panes only) is the sole recovery path after a gap.
