...
Notes:
- OpenCode additionally ships a **skill-dir stub** at `.opencode/skills/<skill>/SKILL.md` alongside the command wrapper (installed by `ait setup` as an OpenCode skill wrapper — see `aitask_setup.sh`). The command wrapper is the auto-discovered entry point and is the canonical stub in the table above; the skill-dir stub carries the same body and dispatches to the same rendered variant. Both are required surfaces.
- **Enforcement.** `tests/test_skill_dispatch_contract.sh` guards this table generically: for every templated skill (discovered via `SKILL.md.j2`, never a hardcoded list) × every agent surface, it parses the stub's Step-3 dispatch target out of the stub body, renders the variant, and asserts that exact path exists and carries the skill's complete authoring-dir closure. It derives the expected rendered path from `agent_skill_dir` in `.aitask-scripts/lib/agent_skills_paths.sh` rather than restating the mapping, so this table remains the single source of truth. (`aitask_skill_verify.sh` is the complementary static check — it greps stub shape but never renders to disk.)
