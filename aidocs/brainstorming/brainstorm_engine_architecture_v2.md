...
This document is the single reference for the proposal-only design engine: data formats, orchestration flow, and agent specifications. The module-decomposition operations (`decompose`, `sync`, `merge`) that let one session grow per-module subgraphs and fast-track individual modules into their own implementation aitasks are specified separately in `aidocs/brainstorming/module_decomposition_design.md`.
...
4. [Structured Sections and Proposals](#4-structured-sections-and-proposals)
...
- **Node Pair:** Every node consists of two files — metadata (YAML) and proposal (Markdown). The metadata is the queryable index; the Markdown file holds the narrative.
...
- Traceability from the exported proposal (and its implementation aitask) back through the design exploration
...
┌──────────────────────────────────────────────────────────┐
│                    TUI (future, t419_6)                   │
│   User commands: explore, compare, synthesize...         │
└──────────────┬───────────────────────────────────────────┘
               │
               ▼
┌──────────────────────────────────────────────────────────┐
...
Brainstorm session data lives on the AgentCrew crew branch alongside the crew coordination files. This unifies all session state (DAG, proposals, agent coordination) in a single git branch, enabling multi-user and multi-PC collaboration via git.
...
- **Session completed (finalize):** The final proposal is exported to `aiplans/` on the main branch (or aitask-data branch). The crew branch is preserved for reference.
...
| Path | Purpose |
|------|---------|
| `.aitask-scripts/brainstorm/brainstorm_dag.py` | DAG operations: `read_node`, `read_proposal`, `_read_graph_state`, path constants (`NODES_DIR`, `PROPOSALS_DIR`) |
...
| `.aitask-scripts/brainstorm/brainstorm_crew.py` | Agent registration (`register_explorer`, `register_comparator`, `register_synthesizer`, `register_initializer`) and `_assemble_input_*` helpers |
| `.aitask-scripts/brainstorm/brainstorm_app.py` | Brainstorm TUI (wizard, node tree, `NodeDetailModal`) |
| `.aitask-scripts/brainstorm/templates/` | Agent work2do templates (`explorer.md`, `comparator.md`, `synthesizer.md`, `initializer.md`) plus the shared include `_section_format.md` |
...
| `completed` | User finalized a design — proposal locked | User runs finalize |
...
4. **`created_by_group`** tracks which AgentCrew operation group produced this node (see Section 5)
5. **Dimension keys** use underscored prefixes: `requirements_fixed`, `requirements_mutable`, `assumption_*`, `component_*`, `tradeoff_pros`, `tradeoff_cons`
6. **New dimensions** can be added freely — the schema is extensible. When a node introduces a new `component_*` or `assumption_*` key not present in the parent, add the dimension name to `br_graph_state.yaml`'s `active_dimensions`
7. **`reference_files`** is a list of references relevant to the proposal — both local file paths and remote URLs. Local paths are relative to the project root. URLs point to external documentation, API specs, or technology references. Each agent updates this list when creating a new node: add references for new components, remove references for removed components. This is the primary mechanism for evolving context across the DAG.

---

## 4. Structured Sections and Proposals

Proposals (`br_proposals/`) are Markdown documents partitioned into **structured sections** delimited by HTML comment markers. Sections are the unit of addressable content: agents can be steered to operate on a subset of them (via `target_sections`), and section-aware TUI widgets render them as a navigable minimap.
...
The brainstorm engine uses AgentCrew as its orchestration layer. Each session creates a persistent AgentCrew crew, and each brainstorm operation (explore, compare, synthesize) registers agents within an operation group.
...
- `<operation>` is one of: `explore`, `compare`, `synthesize`
- `<sequence>` is a zero-padded three-digit counter incremented per operation

Examples: `explore_001`, `explore_002`, `compare_003`, `synthesize_004`
...
The brainstorm engine's memory is entirely external — stored in YAML metadata, Markdown proposals, and codebase files. When an agent is spawned, it knows nothing. The session manager must:
...
**Targeted variant** — when the user selects specific sections for the operation (see §7), an additional block is appended, inlining just the selected sections from the baseline proposal (so the agent does not need to parse the full proposal to find them):
...
**Key point:** The comparator reads only the YAML metadata files and extracts the requested dimension fields. No proposals, no codebase files — this keeps it fast and focused.
...
1. **Operation-specific data** (mandate, merge rules) — always included
2. **Target/baseline node YAML path** — always included
3. **Target/baseline proposal path** — always included
4. **Local reference file paths** — all listed
5. **Remote reference cache paths** — all listed with source URLs
6. **Parent/sibling node YAML paths** — included as supplementary context
7. **Graph state path** — included

The agent's work2do instructs it to read files in priority order and manage its own context budget. If the agent determines it cannot fit all referenced files, it reads metadata files first (compact YAML) and selectively reads full proposals as needed.
...
3. Create subdirectories in crew worktree: `br_nodes/`, `br_proposals/`
...
3. If the baseline node's proposal has structured sections, insert a **section-select step** in the wizard so the user can narrow the exploration to specific sections (skipped when no sections exist)
...
- Input: HEAD node's `.yaml` and `.md` files, the exploration mandate, and — when sections were selected — an inlined `## Targeted Section Content` block. All parallel explorers in the same group receive identical `target_sections`.
...
- Proceed to finalize / fast-track to an implementation aitask
- Further refine via exploration or additional synthesis

### 7.5 Finalize

**Trigger:** User is satisfied with the current head node. Ready to hand the design off to implementation.

**What happens:**
1. Verify the current head has a proposal
2. Update session status to `completed`
3. Stop the AgentCrew crew runner (if running)
4. Export the final proposal to `aiplans/` in aitask format (or hand the design off via fast-track to a linked aitask, which then owns its implementation plan)
5. Link the brainstorm session in the task's metadata

The implementation plan is **not** produced inside brainstorm. It is rebuilt against the live codebase when the exported proposal's aitask is picked for implementation (`/aitask-pick`), which is fresher than any brainstorm-time plan would be. For how a fast-tracked module's as-built design is synced back into the proposal, see `module_decomposition_design.md`.
...
│          │                                               │
│          ▼                                               │
│  Finalize: export proposal / fast-track to aitask        │
└─────────────────────────────────────────────────────────┘
```
...
Section-structured proposals are rendered in three aitasks TUIs (codebrowser, brainstorm, board) by a single shared library: `.aitask-scripts/lib/section_viewer.py`. Centralising the widgets keeps the keyboard contract and visual behaviour identical across hosts and avoids each TUI reinventing minimap rendering.
...
All three hosts bind `V` (shift+v) to open `SectionViewerScreen` for the content they are currently showing:
...
| Brainstorm | `.aitask-scripts/brainstorm/brainstorm_app.py` | Minimap in `NodeDetailModal`'s Proposal tab | `V` → `action_fullscreen_plan` |
...
When a proposal has **no** section markers:
...
- **`aidocs/brainstorming/module_decomposition_design.md`** — Module-decomposition operations (`decompose`, `sync`, `merge`): per-module subgraphs within one session, fast-tracking a module into its own implementation aitask, and syncing the as-implemented design back into the proposal.
...
- **`aidocs/brainstorming/building_an_iterative_ai_design_system.md`** — Original design conversation that defined the conceptual model for the brainstorm engine: Orchestrator-Subagent architecture, DAG structure, flat YAML nodes, subagent roles.
