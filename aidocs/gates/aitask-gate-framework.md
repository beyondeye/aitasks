...
**The check also runs on the read-side introspection and TUI paths (t1416).** `ait gates unlocked` re-lists the gate, the board's In-Flight view reads `awaiting re-sign: <gate>` instead of `all gates pass — archive/re-enter`, and `ait ls` keeps the task's dependents blocked — so no surface a user can reach reports a task ready while the enforcing guard refuses it. All of them go through the same `gate_ledger.demote_stale_signed()` seam the engine uses, so there is one classifier rather than agreeing copies. Two surfaces stay deliberately ledger-only (`archive_status_from_text` and the monitor's compact column) for contractual reasons, registered in a drift guard; the full table, the reasons and the measured costs are in [[gate-guarded-archival]] under "Which surfaces re-validate".
